...
func cmdEdgeRulesCreate(args []string) int {
	...
	// retry (ADR-197 §1). Replay a request that died in transport
	...
	// circuit_breaker (ADR-197 §2). Tunes a breaker that already runs
...
type edgeRuleActionInputs struct {
	...
	RespondStatus int
	RespondBody   string
	// retry (ADR-197 §1). Replay against a different healthy instance.
	...
	RetryBackoffMs          int
	// circuit_breaker (ADR-197 §2). Instance health thresholds. Also all
...
func buildEdgeRuleAction(kind string, in edgeRuleActionInputs) (json.RawMessage, error) {
	...
	case "budget":
		...
		return marshalAction(a)
	case "retry":
		// ADR-197 §1. Every field is optional and the server applies
