...
func cmdPreview(args []string) int {
	if len(args) == 0 {
		PrintUsage(os.Stderr, "usage: gregale preview <create|list|show|destroy> [args]", "preview")
		return 1
	}
	switch args[0] {
	case "create":
	...
	default:
		fmt.Fprintf(os.Stderr, "unknown preview subcommand %q (try: create, list, show, destroy)\n", args[0])
		return 1
	}
}
