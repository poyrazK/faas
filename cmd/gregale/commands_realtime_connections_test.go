...
func TestCmdRealtimeConnectionsListsFilteredInventory(t *testing.T) {
	resetJSONOut(t)
	f := authedFakeAPI(t, `{"connections":[{"id":"conn-a","endpoint_id":"endpoint-1","app_id":"app-1","account_id":"account-1","principal":"user-a","connected_at":"2026-09-17T12:00:00Z","last_seen_at":"2026-09-17T12:02:00Z","expires_at":"2026-09-17T13:00:00Z","channels":["room-a"]}],"limit":25,"truncated":false,"partial":true,"nodes_queried":1,"nodes_unavailable":1}`, http.StatusOK)
	...
	if code := cmdRealtimeConnections([]string{"demo", "endpoint-1", "--channel", "room-a", "--limit", "25"}); code != 0 {
	...
	if f.sawQuery != "channel=room-a&limit=25" {
