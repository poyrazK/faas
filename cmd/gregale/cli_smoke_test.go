// CLI smoke tests for the auth path of `gregale deploy`. Pin the wire
// behaviour the [[cmd-gregale-requireslogin-hermeticity]] memory documents:
...
//
// These are smuggled into cmd/gregale/cli_test.go patterns: hermetic via
...
// TestGregaleCLI_Deploy_NoAuthExitsTwo pins the canonical "no auth" path:
// running `gregale deploy --image ...` with FAAS_TOKEN="" and no token
...
// conformance with the [[cmd-gregale-requireslogin-hermeticity]] project
...
// the CLI ships with.
func TestGregaleCLI_Deploy_NoAuthExitsTwo(t *testing.T) {
...
// TestGregaleCLI_Deploy_HappyPath_ReachesAPID pins the positive path:
...
// documented sequence.
func TestGregaleCLI_Deploy_HappyPath_ReachesAPID(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("XDG_CONFIG_HOME", t.TempDir())
