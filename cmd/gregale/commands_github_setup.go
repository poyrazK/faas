...
func cmdGithubSetup(args []string) int {
	...
	if *dryRun {
		...
		desired := renderGithubSetupWorkflow(positional[0], repoName, branch)
...
func renderGithubSetupWorkflow(app, repo, branch string) string {
	if branch == "" {
		branch = "main"
	}
    ...
    steps:
      - uses: %s/%s@%s
        with:
          ...
          wait: "true"
          wait-timeout: "1200"
`, app, repo, branch, quotedBranch, githubActionRepo, githubActionPath, githubActionVersion, defaultAPIBase, app)
}
...
func renderGithubSetupReceipt(receipt githubSetupReceipt, existing []byte, exists bool) int {
	...
	_, _ = fmt.Fprintf(osStdout, "  repository:        %s\n  production_branch: %s\n  workflow:          %s\n", receipt.Repo, receipt.ProductionBranch, receipt.WorkflowPath)
	switch {
