...
type sidecarPayload struct {
	...
	Scaling                *api.DeploymentScalingRequest   `json:"scaling,omitempty"`
...
func (s *server) handleSourceTarballDeploy(w http.ResponseWriter, r *http.Request, acct state.Account) {
	...
	rolloutReq := &api.CreateDeploymentRequest{Environment: sidecar.Environment, Resources: sidecar.Resources, MaxInstances: sidecar.MaxInstances, Scaling: sidecar.Scaling, TrafficPercent: sidecar.TrafficPercent, Canary: sidecar.Canary, RollbackOn5xx: sidecar.RollbackOn5xx, DisableStartupCPUBoost: sidecar.DisableStartupCPUBoost}
