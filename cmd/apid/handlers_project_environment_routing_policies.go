...
func (s *server) updateProjectEnvironmentRoutingPolicies(w http.ResponseWriter, r *http.Request, acct state.Account) {
	...
	var req api.UpdateProjectEnvironmentRoutingPolicyRequest
	...
	if req.Rules == nil || len(*req.Rules) > maxRules {
		api.WriteProblem(w, api.ErrValidation(fmt.Sprintf("rules must be a complete list of at most %d redirect/rewrite rules on this plan", maxRules)))
		return
	}
	...
	for _, candidate := range *req.Rules {
		if candidate.Kind != string(state.EdgeRuleKindRedirect) && candidate.Kind != string(state.EdgeRuleKindRewrite) {
			api.WriteProblem(w, api.ErrValidation("environment routing policies support only redirect and rewrite rules"))
			return
		}
	...
	policy, err := s.store.PutProjectEnvironmentRoutingPolicy(r.Context(), state.ProjectEnvironmentEdgePolicy{
	...
	if err != nil {
		...
		if errors.Is(err, state.ErrNotFound) {
			...
			api.WriteProblem(w, api.ErrCapacity("could not store environment routing policies"))
		}
		return
	}
	s.audit.Emit(r.Context(), "project.environment.routing_policies.updated", &acct.ID, map[string]any{
	...
	if err := convergence.apply(r.Context(), ""); err != nil {
		...
		api.WriteProblem(w, api.ErrCapacity("routing policy was saved but the serving fleet has not acknowledged it; read the current state before retrying"))
		return
	}
