...
// — the man page `.TH GREGALECTL 1 "date" "gregale" "version"` header reflects the
...
func manSynopsisFlag(w io.Writer, f cliFlag) {
	name := `\-\-` + f.Name
	// The operator manifest predates value placeholders, so retain its
	// existing synopsis convention and render a generic value token.
	value := "value"
	if value != "" {
		if f.Req {
...
func manHeader(w io.Writer, title, subtitle, source string) {
	...
	_, _ = fmt.Fprintf(w, ".TH %s 1 \"%s\" \"%s\" \"%s\"\n", title, time.Now().UTC().Format("2006-01-02"), source, gregalectlVersion)
