...
func TestReleaseBuildsCLITargetsInParallelBeforePublishing(t *testing.T) {
	...
	if matrix < 0 || fanIn < 0 || download < 0 || checksums < 0 || release < 0 {
		t.Fatalf("release workflow is missing matrix fan-in: matrix=%d fanIn=%d download=%d checksums=%d release=%d", matrix, fanIn, download, checksums, release)
	}
	if !(matrix < fanIn && fanIn < download && download < checksums && checksums < release) {
		t.Fatalf("release CLI fan-in is out of order: matrix=%d fanIn=%d download=%d checksums=%d release=%d", matrix, fanIn, download, checksums, release)
	}
