...
func cmdDeployJoinFleet(args []string) int {
	...
	storageEnv := fs.String("storage-env", "", "shared remote storage.env source")
	imagedStorageEnv := fs.String("imaged-storage-env", "", "imaged-only migration lifecycle credential source")
