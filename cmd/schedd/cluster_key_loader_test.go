...
func TestAtomicMinter_RotateSwapsState(t *testing.T) {
	...
	hdrKid, err := decodeJWTHeaderKid(tok)
	if err != nil {
		t.Fatalf("decode JWT header: %v", err)
	}
	if hdrKid != kid2 {
		t.Errorf("expected new kid %s in minted JWT header, got kid=%q, token=%s", kid2, hdrKid, tok)
	}
	if hdrKid == kid {
		// The OLD kid should not be the header kid of the new
		// token. (kid is the kid of priv1; kid2 is the kid of
		// priv2 — they're distinct by construction.)
		t.Errorf("old kid %s should not appear as header kid in new token", kid)
	}
}
...
// decodeJWTHeaderKid extracts the `kid` field from the JWT header
// segment. JWT is `<base64url(header)>.<base64url(payload)>.<sig>`
// — the header is small JSON, base64url-decoded. Used by the
// rotation test to assert the new key id is the one signed into
// the header (a literal substring search would be unreliable
// because the kid is base64-encoded inside the base64-encoded
// header segment, not visible as a plain substring).
func decodeJWTHeaderKid(tok string) (string, error) {
	parts := strings.SplitN(tok, ".", 3)
	if len(parts) < 2 {
		return "", errors.New("jwt: not enough segments")
	}
