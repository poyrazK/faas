...
func registerComputeNode(ctx context.Context, st state.Store, cfg ComputeNodeConfig, targetURL string, detectOverlayIP func(context.Context) (string, error), log *slog.Logger, scheddTargets ...string) (state.ComputeNode, error) {
	...
	//
	// The topology-aware PKI renderer and `gregalectl pki init
	// --box-role=compute-only --cn=...` issue the vmmd leaves with
	// this same rewritten identity. Keeping the rewrite here as the
	// single registration rule means a stale or manually-generated
	// leaf is rejected by the verifier instead of silently accepted.
