...
// customers can paste a row id from the dashboard into `faas build
...
// dispatches `faas build <subcommand>` via this constant.
...
// so the customer's "faas build provanence" typo surfaces
...
// a build id. `faas build provenance <id>` is the customer-facing
// shape; mirrors `faas deployment <id>` for cross-resource parity.
...
func cmdBuildProvenance(args []string) int {
	if len(args) != 1 {
		PrintUsage(os.Stderr, "usage: faas build provenance <id> [-j]", "build")
		return 1
	}
	id := args[0]
	if !buildIDPattern.MatchString(id) {
		PrintUsage(os.Stderr, "usage: faas build provenance <id>   (id is 32 hex chars)", "build")
		return 1
	}
...
// helper so a future `faas build list` (not in this PR) can reuse
