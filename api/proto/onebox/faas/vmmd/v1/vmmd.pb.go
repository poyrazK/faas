...
type AppSpec struct {
	...
	// "layer-sidecar-1") + storage_key + dependency gates. cmd/env/port
	// are read by guest-init from the workload.json written to the
	// sidecar's drive1 (ADR-069 §Sidecar staging) — they are NOT on the
	// wire because vmmd never reads them; guest-init does.
	//
	...
	// private_network_id identifies a Gregale-owned network. Empty preserves
	// the provider-verified route-only path for older callers.
	PrivateNetworkId string `protobuf:"bytes,20,opt,name=private_network_id,json=privateNetworkId,proto3" json:"private_network_id,omitempty"`
	// private_network_address is the stable app member address allocated by
...
func (x *AppSpec) GetPrivateNetworkId() string {
	if x != nil {
...
func (x *AppSpec) GetPrivateNetworkAddress() string {
	if x != nil {
...
func (x *AppSpec) GetPrivateNetworkFirewallRules() []*PrivateNetworkFirewallRule {
	if x != nil {
...
func (x *SidecarSpec) GetDependsOn() []*WorkloadDependency {
	if x != nil {
		return x.DependsOn
	}
	return nil
...
// WorkloadDependency is the compact gRPC representation of
// api.WorkloadDependency. Name is "main" or another sidecar name; condition
// is started, healthy, or completed_successfully.
...
func (x *WorkloadDependency) Reset() {
	*x = WorkloadDependency{}
	mi := &file_onebox_faas_vmmd_v1_vmmd_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WorkloadDependency) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkloadDependency) ProtoMessage() {}

func (x *WorkloadDependency) ProtoReflect() protoreflect.Message {
	mi := &file_onebox_faas_vmmd_v1_vmmd_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use WorkloadDependency.ProtoReflect.Descriptor instead.
func (*WorkloadDependency) Descriptor() ([]byte, []int) {
	return file_onebox_faas_vmmd_v1_vmmd_proto_rawDescGZIP(), []int{4}
}

func (x *WorkloadDependency) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
...
func (x *WorkloadDependency) GetCondition() string {
	if x != nil {
		return x.Condition
	}
	return ""
...
func (x *SnapshotRef) GetNetworkless() bool {
	if x != nil {
...
type CreateFromSnapshotRequest struct {
	...
	// Build carries the builder-VM specifics when a warm builder snapshot is
...
func (x *CreateFromSnapshotRequest) GetBuild() *BuildSpec {
	if x != nil {
		return x.Build
	}
	return nil
...
type InstanceStats struct {
	...
	// ADR-046: cumulative bytes observed on
	...
	// poller.go) forwards the cumulative snapshot through
	// pkg/scheddgrpc.ListInstanceStats; meterd computes the interval
	// delta before appending
	// to usage_minutes.net_tx_bytes. Source: cmd/vmmd/network_poller.go +
	...
	// Cumulative root-side vethHost.tx_bytes for this instance
	// (root → guest = ingress). Same wrapper/regression contract as
	// net_tx_bytes; meterd computes interval deltas from both counters.
	NetRxBytes *wrapperspb.Int64Value `protobuf:"bytes,13,opt,name=net_rx_bytes,json=netRxBytes,proto3" json:"net_rx_bytes,omitempty"`
	// Writable application filesystem usage sampled by guest-init. Wrapper
	// fields keep an unavailable sample distinct from a real zero.
	DiskUsedBytes     *wrapperspb.Int64Value `protobuf:"bytes,14,opt,name=disk_used_bytes,json=diskUsedBytes,proto3" json:"disk_used_bytes,omitempty"`
	DiskCapacityBytes *wrapperspb.Int64Value `protobuf:"bytes,15,opt,name=disk_capacity_bytes,json=diskCapacityBytes,proto3" json:"disk_capacity_bytes,omitempty"`
...
func (x *InstanceStats) GetNetRxBytes() *wrapperspb.Int64Value {
	if x != nil {
		return x.NetRxBytes
	}
	return nil
...
func (x *InstanceStats) GetDiskUsedBytes() *wrapperspb.Int64Value {
	if x != nil {
...
func (x *InstanceStats) GetDiskCapacityBytes() *wrapperspb.Int64Value {
	if x != nil {
...
func (x *InstanceStats) GetFlowSummaries() []*FlowSummary {
	if x != nil {
...
//
// The address is resolved by schedd, not carried as a hostname, because
// nftables matches packets. A DNS change while a circuit is open is picked
// up at the next half-open probe, which re-resolves; that staleness window
// is bounded by the probe interval and is part of why half-open exists.
//
// §11: the plaintext upstream host never appears on this wire. schedd
// resolves it locally from the data_upstreams row it already holds; the
// customer-facing surface and every metric label carry only
// host_redacted_hash.
type UpdateEgressCircuitRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AppId         string                 `protobuf:"bytes,1,opt,name=app_id,json=appId,proto3" json:"app_id,omitempty"`
	Circuits      []*EgressCircuitTarget `protobuf:"bytes,2,rep,name=circuits,proto3" json:"circuits,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *UpdateEgressCircuitRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *UpdateEgressCircuitRequest) GetCircuits() []*EgressCircuitTarget {
	if x != nil {
		return x.Circuits
	}
	return nil
...
// port whose new connections must fail fast.
type EgressCircuitTarget struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// addr is a resolved IPv4 or IPv6 address in its textual form. vmmd
	// renders it into an nftables set element; a value that does not parse is
	// skipped rather than emitted, because one malformed element fails the
	// whole nft batch and would leave the tenant's firewall in whatever state
	// the partial batch produced.
	Addr string `protobuf:"bytes,1,opt,name=addr,proto3" json:"addr,omitempty"`
	// port is the upstream TCP port (1..65535).
...
func (x *EgressCircuitTarget) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *EgressCircuitTarget) GetPort() uint32 {
	if x != nil {
		return x.Port
	}
	return 0
...
// UpdateEgressCircuitAck is the empty success response. On failure vmmd
// returns a gRPC status: Internal when the nft batch failed for a live
// instance. Unlike UpdateEgressAllowlist there is no partial-revert path —
// the operation is a flush plus adds against a set that is re-derived on
// every reconcile, so a failure is corrected by the next push rather than by
// unwinding this one.
type UpdateEgressCircuitAck struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *UpdateEgressCircuitAck) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
// for one app. The caller validates the attachment before sending this hot-path
// update; vmmd validates again at the trust boundary.
type UpdatePrivateNetworkRequest struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	AppId               string                 `protobuf:"bytes,1,opt,name=app_id,json=appId,proto3" json:"app_id,omitempty"`
	PrivateNetworkCidrs []string               `protobuf:"bytes,2,rep,name=private_network_cidrs,json=privateNetworkCidrs,proto3" json:"private_network_cidrs,omitempty"`
	// Optional Gregale-owned identity. When both fields are present vmmd
	// attaches the live instances to the node-local gpn-* bridge; empty keeps
	...
	PrivateNetworkFirewallRules []*PrivateNetworkFirewallRule `protobuf:"bytes,6,rep,name=private_network_firewall_rules,json=privateNetworkFirewallRules,proto3" json:"private_network_firewall_rules,omitempty"`
	unknownFields               protoimpl.UnknownFields
	sizeCache                   protoimpl.SizeCache
}
...
func (x *UpdatePrivateNetworkRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *UpdatePrivateNetworkRequest) GetPrivateNetworkCidrs() []string {
	if x != nil {
		return x.PrivateNetworkCidrs
	}
	return nil
...
func (x *UpdatePrivateNetworkRequest) GetPrivateNetworkId() string {
	if x != nil {
		return x.PrivateNetworkId
	}
	return ""
...
func (x *UpdatePrivateNetworkRequest) GetPrivateNetworkAddress() string {
	if x != nil {
		return x.PrivateNetworkAddress
	}
	return ""
...
func (x *UpdatePrivateNetworkRequest) GetPrivateNetworkFirewallRules() []*PrivateNetworkFirewallRule {
	if x != nil {
		return x.PrivateNetworkFirewallRules
	}
	return nil
...
type UpdatePrivateNetworkAck struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *UpdatePrivateNetworkAck) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
type WaitBuilderReadyRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Instance      string                 `protobuf:"bytes,1,opt,name=instance,proto3" json:"instance,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
...
func (x *WaitBuilderReadyRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
type WaitBuilderReadyResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Instance      string                 `protobuf:"bytes,1,opt,name=instance,proto3" json:"instance,omitempty"`
...
func (x *WaitBuilderReadyResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *WaitBuilderReadyResponse) GetReady() bool {
	if x != nil {
		return x.Ready
	}
	return false
...
func (x *WaitBuilderReadyResponse) GetExitCode() int32 {
	if x != nil {
		return x.ExitCode
	}
	return 0
...
type DeleteWarmSnapshotRequest struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	StorageKey        string                 `protobuf:"bytes,1,opt,name=storage_key,json=storageKey,proto3" json:"storage_key,omitempty"`
	VmstateStorageKey string                 `protobuf:"bytes,2,opt,name=vmstate_storage_key,json=vmstateStorageKey,proto3" json:"vmstate_storage_key,omitempty"`
	unknownFields     protoimpl.UnknownFields
...
func (x *DeleteWarmSnapshotRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *DeleteWarmSnapshotRequest) GetVmstateStorageKey() string {
	if x != nil {
		return x.VmstateStorageKey
	}
	return ""
...
type DeleteWarmSnapshotResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
...
func (x *DeleteWarmSnapshotResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
// ExecuteExecution is the post-restore host/guest execution envelope. The
// request is validated again at vmmd even though apid/schedd already admitted
// it: vmmd is the last trust boundary before untrusted bytes enter a guest.
...
func (x *ExecuteExecutionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ExecuteExecutionRequest) GetVersion() uint32 {
	if x != nil {
		return x.Version
	}
	return 0
...
func (x *ExecuteExecutionRequest) GetExecutionId() string {
	if x != nil {
...
func (x *ExecuteExecutionRequest) GetRuntime() string {
	if x != nil {
		return x.Runtime
	}
	return ""
...
func (x *ExecuteExecutionRequest) GetSource() string {
	if x != nil {
		return x.Source
	}
	return ""
...
func (x *ExecuteExecutionRequest) GetInput() []byte {
	if x != nil {
		return x.Input
	}
	return nil
...
func (x *ExecuteExecutionRequest) GetTimeoutMs() int32 {
	if x != nil {
		return x.TimeoutMs
	}
	return 0
...
func (x *ExecuteExecutionRequest) GetMaxOutputBytes() int32 {
	if x != nil {
...
func (x *ExecuteExecutionRequest) GetNetworkMode() string {
	if x != nil {
...
type ExecuteExecutionResponse struct {
	...
	OutputTruncated bool                   `protobuf:"varint,4,opt,name=output_truncated,json=outputTruncated,proto3" json:"output_truncated,omitempty"`
	ExitCode        *wrapperspb.Int32Value `protobuf:"bytes,5,opt,name=exit_code,json=exitCode,proto3" json:"exit_code,omitempty"`
...
func (x *ExecuteExecutionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ExecuteExecutionResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
...
func (x *ExecuteExecutionResponse) GetResult() []byte {
	if x != nil {
		return x.Result
	}
	return nil
...
func (x *ExecuteExecutionResponse) GetOutputTruncated() bool {
	if x != nil {
...
func (x *ExecuteExecutionResponse) GetExitCode() *wrapperspb.Int32Value {
	if x != nil {
		return x.ExitCode
	}
	return nil
...
func (x *ExecuteExecutionResponse) GetFailureCode() string {
	if x != nil {
...
func (x *ExecuteExecutionResponse) GetFailureMessage() string {
	if x != nil {
...
func (x *ExecuteExecutionResponse) GetStdout() []byte {
	if x != nil {
		return x.Stdout
	}
	return nil
...
func (x *ExecuteExecutionResponse) GetStderr() []byte {
	if x != nil {
		return x.Stderr
	}
	return nil
...
func (x *ExecuteExecutionResponse) GetWallTimeMs() int64 {
	if x != nil {
		return x.WallTimeMs
	}
	return 0
...
func (x *ExecuteExecutionResponse) GetCpuTimeMs() int64 {
	if x != nil {
		return x.CpuTimeMs
	}
	return 0
...
func (x *ExecuteExecutionResponse) GetPeakMemoryMb() int32 {
	if x != nil {
...
type RestoreExecutionRequest struct {
	...
	Runtime       string                 `protobuf:"bytes,4,opt,name=runtime,proto3" json:"runtime,omitempty"`
	KernelKey     string                 `protobuf:"bytes,5,opt,name=kernel_key,json=kernelKey,proto3" json:"kernel_key,omitempty"`
...
func (x *RestoreExecutionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *RestoreExecutionRequest) GetAccountId() string {
	if x != nil {
		return x.AccountId
	}
	return ""
...
func (x *RestoreExecutionRequest) GetPlan() string {
	if x != nil {
		return x.Plan
	}
	return ""
...
func (x *RestoreExecutionRequest) GetRuntime() string {
	if x != nil {
		return x.Runtime
	}
	return ""
...
func (x *RestoreExecutionRequest) GetKernelKey() string {
	if x != nil {
		return x.KernelKey
	}
	return ""
...
func (x *RestoreExecutionRequest) GetBaseKey() string {
	if x != nil {
		return x.BaseKey
	}
	return ""
...
func (x *RestoreExecutionRequest) GetLayerKey() string {
	if x != nil {
		return x.LayerKey
	}
	return ""
...
func (x *RestoreExecutionRequest) GetSnapshot() *SnapshotRef {
	if x != nil {
		return x.Snapshot
	}
	return nil
...
func (x *RestoreExecutionRequest) GetVcpuCount() int32 {
	if x != nil {
		return x.VcpuCount
	}
	return 0
...
func (x *RestoreExecutionRequest) GetMemSizeMib() int32 {
	if x != nil {
		return x.MemSizeMib
	}
	return 0
...
func (x *RestoreExecutionRequest) GetCpuMillicores() int32 {
	if x != nil {
		return x.CpuMillicores
	}
	return 0
...
type RestoreExecutionResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Instance        string                 `protobuf:"bytes,1,opt,name=instance,proto3" json:"instance,omitempty"`
	LeaseUid        int32                  `protobuf:"varint,2,opt,name=lease_uid,json=leaseUid,proto3" json:"lease_uid,omitempty"`
	Method          WakeMethod             `protobuf:"varint,3,opt,name=method,proto3,enum=onebox.faas.vmmd.v1.WakeMethod" json:"method,omitempty"`
	RequestedMethod WakeMethod             `protobuf:"varint,4,opt,name=requested_method,json=requestedMethod,proto3,enum=onebox.faas.vmmd.v1.WakeMethod" json:"requested_method,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
func (x *RestoreExecutionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *RestoreExecutionResponse) GetLeaseUid() int32 {
	if x != nil {
		return x.LeaseUid
	}
	return 0
...
func (x *RestoreExecutionResponse) GetMethod() WakeMethod {
	if x != nil {
		return x.Method
	}
	return WakeMethod_WAKE_COLD_BOOT
...
func (x *RestoreExecutionResponse) GetRequestedMethod() WakeMethod {
	if x != nil {
		return x.RequestedMethod
	}
	return WakeMethod_WAKE_COLD_BOOT
...
func (x *ReconcilePrivateNetworkFabricRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ReconcilePrivateNetworkFabricRequest) GetNetworkId() string {
	if x != nil {
		return x.NetworkId
	}
	return ""
...
func (x *ReconcilePrivateNetworkFabricRequest) GetRegion() string {
	if x != nil {
		return x.Region
	}
	return ""
...
func (x *ReconcilePrivateNetworkFabricRequest) GetCidr() string {
	if x != nil {
		return x.Cidr
	}
	return ""
...
func (x *ReconcilePrivateNetworkFabricAck) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
// RemovePrivateNetworkFabricRequest carries the identity needed to remove a
// node-local Gregale-owned network fabric after its durable control-plane row
// has been deleted.
type RemovePrivateNetworkFabricRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AccountId     string                 `protobuf:"bytes,1,opt,name=account_id,json=accountId,proto3" json:"account_id,omitempty"`
	NetworkId     string                 `protobuf:"bytes,2,opt,name=network_id,json=networkId,proto3" json:"network_id,omitempty"`
	Region        string                 `protobuf:"bytes,3,opt,name=region,proto3" json:"region,omitempty"`
...
func (x *RemovePrivateNetworkFabricRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *RemovePrivateNetworkFabricRequest) GetNetworkId() string {
	if x != nil {
		return x.NetworkId
	}
	return ""
...
func (x *RemovePrivateNetworkFabricRequest) GetRegion() string {
	if x != nil {
		return x.Region
	}
	return ""
...
func (x *RemovePrivateNetworkFabricRequest) GetCidr() string {
	if x != nil {
		return x.Cidr
	}
	return ""
...
// RemovePrivateNetworkFabricAck confirms an idempotent fabric teardown.
type RemovePrivateNetworkFabricAck struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *RemovePrivateNetworkFabricAck) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
// responsible for selecting a listener declared by the app manifest; vmmd
// only resolves the instance-local network namespace and dials the supplied
// guest port.
type ForwardTCPRequestInit struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Instance string                 `protobuf:"bytes,1,opt,name=instance,proto3" json:"instance,omitempty"`
	Port     uint32                 `protobuf:"varint,2,opt,name=port,proto3" json:"port,omitempty"`
	// Optional per-direction byte cap. Zero means the transport default
	// selected by the receiving vmmd.
...
func (x *ForwardTCPRequestInit) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ForwardTCPRequestInit) GetPort() uint32 {
	if x != nil {
		return x.Port
	}
	return 0
...
func (x *ForwardTCPRequestInit) GetMaxBytes() int64 {
	if x != nil {
		return x.MaxBytes
	}
	return 0
...
// ForwardTCPRequest carries protocol-neutral bytes. The init frame MUST be
// first; subsequent frames are copied verbatim to the guest socket.
type ForwardTCPRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Frame:
	//
	//	*ForwardTCPRequest_Init
	//	*ForwardTCPRequest_BodyChunk
	Frame         isForwardTCPRequest_Frame `protobuf_oneof:"frame"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *ForwardTCPRequest) GetInit() *ForwardTCPRequestInit {
	if x != nil {
		if x, ok := x.Frame.(*ForwardTCPRequest_Init); ok {
			return x.Init
		}
	}
	return nil
...
func (x *ForwardTCPRequest) GetBodyChunk() []byte {
	if x != nil {
		if x, ok := x.Frame.(*ForwardTCPRequest_BodyChunk); ok {
			return x.BodyChunk
		}
	}
	return nil
...
type isForwardTCPRequest_Frame interface {
	isForwardTCPRequest_Frame()
}

type ForwardTCPRequest_Init struct {
	Init *ForwardTCPRequestInit `protobuf:"bytes,1,opt,name=init,proto3,oneof"`
}

type ForwardTCPRequest_BodyChunk struct {
	BodyChunk []byte `protobuf:"bytes,2,opt,name=body_chunk,json=bodyChunk,proto3,oneof"`
}

func (*ForwardTCPRequest_Init) isForwardTCPRequest_Frame() {}

func (*ForwardTCPRequest_BodyChunk) isForwardTCPRequest_Frame() {}

// ForwardTCPResponseInit is sent once before response bytes. A non-empty
// error means the guest socket could not be opened and no bytes follow.
type ForwardTCPResponseInit struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
...
func (x *ForwardTCPResponseInit) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
// ForwardTCPResponse carries protocol-neutral bytes read from the guest
// socket. End-of-stream is represented by the server closing the gRPC stream.
type ForwardTCPResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Frame:
	//
	//	*ForwardTCPResponse_Init
	//	*ForwardTCPResponse_BodyChunk
	Frame         isForwardTCPResponse_Frame `protobuf_oneof:"frame"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *ForwardTCPResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ForwardTCPResponse) GetInit() *ForwardTCPResponseInit {
	if x != nil {
		if x, ok := x.Frame.(*ForwardTCPResponse_Init); ok {
			return x.Init
		}
	}
	return nil
...
func (x *ForwardTCPResponse) GetBodyChunk() []byte {
	if x != nil {
		if x, ok := x.Frame.(*ForwardTCPResponse_BodyChunk); ok {
			return x.BodyChunk
		}
	}
	return nil
...
type isForwardTCPResponse_Frame interface {
	isForwardTCPResponse_Frame()
}

type ForwardTCPResponse_Init struct {
	Init *ForwardTCPResponseInit `protobuf:"bytes,1,opt,name=init,proto3,oneof"`
}

type ForwardTCPResponse_BodyChunk struct {
	BodyChunk []byte `protobuf:"bytes,2,opt,name=body_chunk,json=bodyChunk,proto3,oneof"`
}

func (*ForwardTCPResponse_Init) isForwardTCPResponse_Frame() {}

func (*ForwardTCPResponse_BodyChunk) isForwardTCPResponse_Frame() {}

// FlowSummary is one bounded endpoint aggregate observed in conntrack for an
// instance. It is an endpoint snapshot, not packet byte or latency accounting.
...
func (x *FlowSummary) GetRemoteIp() string {
	if x != nil {
		return x.RemoteIp
	}
	return ""
...
func (x *FlowSummary) GetRemotePort() uint32 {
	if x != nil {
		return x.RemotePort
	}
	return 0
...
func (x *FlowSummary) GetState() string {
	if x != nil {
		return x.State
	}
	return ""
...
func (x *FlowSummary) GetDirection() string {
	if x != nil {
		return x.Direction
	}
	return ""
...
func (x *FlowSummary) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
...
// PrivateNetworkFirewallRule is the wire representation of one private
// network allow rule. CIDRs are source ranges for ingress and destination
// ranges for egress; ports are single values or inclusive ranges.
...
func (x *PrivateNetworkFirewallRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *PrivateNetworkFirewallRule) GetProtocol() string {
	if x != nil {
		return x.Protocol
	}
	return ""
...
func (x *PrivateNetworkFirewallRule) GetCidrs() []string {
	if x != nil {
		return x.Cidrs
	}
	return nil
...
func (x *PrivateNetworkFirewallRule) GetPorts() []string {
	if x != nil {
		return x.Ports
	}
	return nil
...
func (x *ResumeWarmInstanceRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ResumeWarmInstanceResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
// deployment-attached one-shot command. App carries the same immutable
// runtime, environment, secret, and network projection as an ordinary wake.
type RestoreAppTaskRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Instance      string                 `protobuf:"bytes,1,opt,name=instance,proto3" json:"instance,omitempty"`
	App           *AppSpec               `protobuf:"bytes,2,opt,name=app,proto3" json:"app,omitempty"`
	Plan          string                 `protobuf:"bytes,3,opt,name=plan,proto3" json:"plan,omitempty"`
...
func (x *RestoreAppTaskRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *RestoreAppTaskRequest) GetApp() *AppSpec {
	if x != nil {
		return x.App
	}
	return nil
...
func (x *RestoreAppTaskRequest) GetPlan() string {
	if x != nil {
		return x.Plan
	}
	return ""
...
func (x *RestoreAppTaskRequest) GetAccountId() string {
	if x != nil {
		return x.AccountId
	}
	return ""
...
func (x *RestoreAppTaskRequest) GetDeploymentId() string {
	if x != nil {
		return x.DeploymentId
	}
	return ""
...
func (x *RestoreAppTaskResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *RestoreAppTaskResponse) GetLeaseUid() int32 {
	if x != nil {
		return x.LeaseUid
	}
	return 0
...
func (x *RestoreAppTaskResponse) GetMethod() WakeMethod {
	if x != nil {
		return x.Method
	}
	return WakeMethod_WAKE_COLD_BOOT
...
// RestoreAppTask.
type ExecuteAppTaskRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Instance       string                 `protobuf:"bytes,1,opt,name=instance,proto3" json:"instance,omitempty"`
	Version        uint32                 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
...
func (x *ExecuteAppTaskRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ExecuteAppTaskRequest) GetVersion() uint32 {
	if x != nil {
		return x.Version
	}
	return 0
...
func (x *ExecuteAppTaskRequest) GetTaskId() string {
	if x != nil {
		return x.TaskId
	}
	return ""
...
func (x *ExecuteAppTaskRequest) GetCommand() []string {
	if x != nil {
		return x.Command
	}
	return nil
...
func (x *ExecuteAppTaskRequest) GetCommandShell() bool {
	if x != nil {
...
func (x *ExecuteAppTaskRequest) GetTimeoutSeconds() int32 {
	if x != nil {
...
type ExecuteAppTaskResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	...
	ExitCode        *wrapperspb.Int32Value `protobuf:"bytes,4,opt,name=exit_code,json=exitCode,proto3" json:"exit_code,omitempty"`
...
func (x *ExecuteAppTaskResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ExecuteAppTaskResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
...
func (x *ExecuteAppTaskResponse) GetOutputTruncated() bool {
	if x != nil {
		return x.OutputTruncated
	}
	return false
...
func (x *ExecuteAppTaskResponse) GetExitCode() *wrapperspb.Int32Value {
	if x != nil {
		return x.ExitCode
	}
	return nil
...
func (x *ExecuteAppTaskResponse) GetFailureCode() string {
	if x != nil {
		return x.FailureCode
	}
	return ""
...
func (x *ExecuteAppTaskResponse) GetFailureMessage() string {
	if x != nil {
		return x.FailureMessage
	}
	return ""
...
func (x *ExecuteAppTaskResponse) GetStdout() []byte {
	if x != nil {
		return x.Stdout
	}
	return nil
...
func (x *ExecuteAppTaskResponse) GetStderr() []byte {
	if x != nil {
		return x.Stderr
	}
	return nil
...
type ExecuteAppTaskEvent struct {
	...
	// Types that are valid to be assigned to Frame:
	//
	//	*ExecuteAppTaskEvent_Output
	//	*ExecuteAppTaskEvent_Terminal
	Frame         isExecuteAppTaskEvent_Frame `protobuf_oneof:"frame"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *ExecuteAppTaskEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ExecuteAppTaskEvent) GetOutput() *ExecuteAppTaskOutputChunk {
	if x != nil {
		if x, ok := x.Frame.(*ExecuteAppTaskEvent_Output); ok {
			return x.Output
		}
	}
	return nil
...
func (x *ExecuteAppTaskEvent) GetTerminal() *ExecuteAppTaskResponse {
	if x != nil {
		if x, ok := x.Frame.(*ExecuteAppTaskEvent_Terminal); ok {
			return x.Terminal
		}
	}
	return nil
...
type isExecuteAppTaskEvent_Frame interface {
	isExecuteAppTaskEvent_Frame()
}

type ExecuteAppTaskEvent_Output struct {
	Output *ExecuteAppTaskOutputChunk `protobuf:"bytes,1,opt,name=output,proto3,oneof"`
}

type ExecuteAppTaskEvent_Terminal struct {
	Terminal *ExecuteAppTaskResponse `protobuf:"bytes,2,opt,name=terminal,proto3,oneof"`
}

func (*ExecuteAppTaskEvent_Output) isExecuteAppTaskEvent_Frame() {}

func (*ExecuteAppTaskEvent_Terminal) isExecuteAppTaskEvent_Frame() {}

type ExecuteAppTaskOutputChunk struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// stream is one of "stdout" or "stderr".
	Stream        string `protobuf:"bytes,1,opt,name=stream,proto3" json:"stream,omitempty"`
	Chunk         []byte `protobuf:"bytes,2,opt,name=chunk,proto3" json:"chunk,omitempty"`
...
func (x *ExecuteAppTaskOutputChunk) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *ExecuteAppTaskOutputChunk) GetChunk() []byte {
	if x != nil {
		return x.Chunk
	}
	return nil
...
const file_onebox_faas_vmmd_v1_vmmd_proto_rawDesc = "" +
	...
	"\x13request_count_total\x18\f \x01(\v2\x1b.google.protobuf.Int64ValueR\x11requestCountTotal\x12=\n" +
	"\fnet_rx_bytes\x18\r \x01(\v2\x1b.google.protobuf.Int64ValueR\n" +
	"netRxBytes\x12C\n" +
	...
	"\x1dprivate_network_allowed_cidrs\x18\x05 \x03(\tR\x1aprivateNetworkAllowedCidrs\x12t\n" +
	"\x1eprivate_network_firewall_rules\x18\x06 \x03(\v2/.onebox.faas.vmmd.v1.PrivateNetworkFirewallRuleR\x1bprivateNetworkFirewallRules\"\x19\n" +
	...
	"body_chunk\x18\x02 \x01(\fH\x00R\tbodyChunkB\a\n" +
	"\x05frame\"5\n" +
	"\x17WaitBuilderReadyRequest\x12\x1a\n" +
	"\binstance\x18\x01 \x01(\tR\binstance\"i\n" +
	"\x18WaitBuilderReadyResponse\x12\x1a\n" +
	"\binstance\x18\x01 \x01(\tR\binstance\x12\x14\n" +
	"\x05ready\x18\x02 \x01(\bR\x05ready\x12\x1b\n" +
	"\texit_code\x18\x03 \x01(\x05R\bexitCode\"l\n" +
	"\x19DeleteWarmSnapshotRequest\x12\x1f\n" +
	"\vstorage_key\x18\x01 \x01(\tR\n" +
	"storageKey\x12.\n" +
	...
	"\x05input\x18\x06 \x01(\fR\x05input\x12\x1d\n" +
	"\n" +
	...
	"\x10output_truncated\x18\x04 \x01(\bR\x0foutputTruncated\x128\n" +
	"\texit_code\x18\x05 \x01(\v2\x1b.google.protobuf.Int32ValueR\bexitCode\x12!\n" +
	"\ffailure_code\x18\x06 \x01(\tR\vfailureCode\x12'\n" +
	"\x0ffailure_message\x18\a \x01(\tR\x0efailureMessage\x12\x16\n" +
	...
	"\n" +
	"kernel_key\x18\x05 \x01(\tR\tkernelKey\x12\x19\n" +
	"\bbase_key\x18\x06 \x01(\tR\abaseKey\x12\x1b\n" +
	"\tlayer_key\x18\a \x01(\tR\blayerKey\x12<\n" +
	"\bsnapshot\x18\b \x01(\v2 .onebox.faas.vmmd.v1.SnapshotRefR\bsnapshot\x12\x1d\n" +
	"\n" +
	"vcpu_count\x18\t \x01(\x05R\tvcpuCount\x12 \n" +
	"\fmem_size_mib\x18\n" +
	" \x01(\x05R\n" +
	"memSizeMib\x12%\n" +
	...
	"\x17observed_peer_addresses\x18\x05 \x03(\tR\x15observedPeerAddresses\"\x8d\x01\n" +
	"!RemovePrivateNetworkFabricRequest\x12\x1d\n" +
	"\n" +
	"account_id\x18\x01 \x01(\tR\taccountId\x12\x1d\n" +
	"\n" +
	"network_id\x18\x02 \x01(\tR\tnetworkId\x12\x16\n" +
	"\x06region\x18\x03 \x01(\tR\x06region\x12\x12\n" +
	"\x04cidr\x18\x04 \x01(\tR\x04cidr\"\x1f\n" +
	"\x1dRemovePrivateNetworkFabricAck\"d\n" +
	"\x15ForwardTCPRequestInit\x12\x1a\n" +
	"\binstance\x18\x01 \x01(\tR\binstance\x12\x12\n" +
	"\x04port\x18\x02 \x01(\rR\x04port\x12\x1b\n" +
	"\tmax_bytes\x18\x03 \x01(\x03R\bmaxBytes\"\x7f\n" +
	"\x11ForwardTCPRequest\x12@\n" +
	"\x04init\x18\x01 \x01(\v2*.onebox.faas.vmmd.v1.ForwardTCPRequestInitH\x00R\x04init\x12\x1f\n" +
	"\n" +
	"body_chunk\x18\x02 \x01(\fH\x00R\tbodyChunkB\a\n" +
	"\x05frame\".\n" +
	"\x16ForwardTCPResponseInit\x12\x14\n" +
	"\x05error\x18\x01 \x01(\tR\x05error\"\x81\x01\n" +
	"\x12ForwardTCPResponse\x12A\n" +
	"\x04init\x18\x01 \x01(\v2+.onebox.faas.vmmd.v1.ForwardTCPResponseInitH\x00R\x04init\x12\x1f\n" +
	"\n" +
	"body_chunk\x18\x02 \x01(\fH\x00R\tbodyChunkB\a\n" +
	"\x05frame\"\xb1\x01\n" +
	"\vFlowSummary\x12\x1a\n" +
	...
	"\x05count\x18\x06 \x01(\x03R\x05count\"\x82\x01\n" +
	"\x1aPrivateNetworkFirewallRule\x12\x1c\n" +
	...
	"\x10output_truncated\x18\x03 \x01(\bR\x0foutputTruncated\x128\n" +
	"\texit_code\x18\x04 \x01(\v2\x1b.google.protobuf.Int32ValueR\bexitCode\x12!\n" +
	...
	"\x12ResumeWarmInstance\x12..onebox.faas.vmmd.v1.ResumeWarmInstanceRequest\x1a/.onebox.faas.vmmd.v1.ResumeWarmInstanceResponse\x12o\n" +
	"\x10WaitBuilderReady\x12,.onebox.faas.vmmd.v1.WaitBuilderReadyRequest\x1a-.onebox.faas.vmmd.v1.WaitBuilderReadyResponse\x12u\n" +
	"\x12DeleteWarmSnapshot\x12..onebox.faas.vmmd.v1.DeleteWarmSnapshotRequest\x1a/.onebox.faas.vmmd.v1.DeleteWarmSnapshotResponse\x12i\n" +
	...
	"\x1dReconcilePrivateNetworkFabric\x129.onebox.faas.vmmd.v1.ReconcilePrivateNetworkFabricRequest\x1a5.onebox.faas.vmmd.v1.ReconcilePrivateNetworkFabricAck\x12\x88\x01\n" +
	"\x1aRemovePrivateNetworkFabric\x126.onebox.faas.vmmd.v1.RemovePrivateNetworkFabricRequest\x1a2.onebox.faas.vmmd.v1.RemovePrivateNetworkFabricAck\x12f\n" +
...
var file_onebox_faas_vmmd_v1_vmmd_proto_msgTypes = make([]protoimpl.MessageInfo, 99)
var file_onebox_faas_vmmd_v1_vmmd_proto_goTypes = []any{
	...
	(*WorkloadDependency)(nil),                   // 5: onebox.faas.vmmd.v1.WorkloadDependency
	(*SnapshotRef)(nil),                          // 6: onebox.faas.vmmd.v1.SnapshotRef
	(*WakeResponse)(nil),                         // 7: onebox.faas.vmmd.v1.WakeResponse
	(*CreateFromSnapshotRequest)(nil),            // 8: onebox.faas.vmmd.v1.CreateFromSnapshotRequest
	(*CreateColdBootRequest)(nil),                // 9: onebox.faas.vmmd.v1.CreateColdBootRequest
	(*JobColdBootRequest)(nil),                   // 10: onebox.faas.vmmd.v1.JobColdBootRequest
	(*JobColdBootResponse)(nil),                  // 11: onebox.faas.vmmd.v1.JobColdBootResponse
	(*WaitJobExitRequest)(nil),                   // 12: onebox.faas.vmmd.v1.WaitJobExitRequest
	(*JobExitResponse)(nil),                      // 13: onebox.faas.vmmd.v1.JobExitResponse
	(*BuildSpec)(nil),                            // 14: onebox.faas.vmmd.v1.BuildSpec
	(*PauseAndSnapshotRequest)(nil),              // 15: onebox.faas.vmmd.v1.PauseAndSnapshotRequest
	(*SnapshotResponse)(nil),                     // 16: onebox.faas.vmmd.v1.SnapshotResponse
	(*WarmSnapshotRequest)(nil),                  // 17: onebox.faas.vmmd.v1.WarmSnapshotRequest
	(*FrameworkReadyRequest)(nil),                // 18: onebox.faas.vmmd.v1.FrameworkReadyRequest
	(*FrameworkReadyResponse)(nil),               // 19: onebox.faas.vmmd.v1.FrameworkReadyResponse
	(*DestroyRequest)(nil),                       // 20: onebox.faas.vmmd.v1.DestroyRequest
	(*DestroyResponse)(nil),                      // 21: onebox.faas.vmmd.v1.DestroyResponse
	(*StopInstanceRequest)(nil),                  // 22: onebox.faas.vmmd.v1.StopInstanceRequest
	(*StopInstanceResponse)(nil),                 // 23: onebox.faas.vmmd.v1.StopInstanceResponse
	(*StatsRequest)(nil),                         // 24: onebox.faas.vmmd.v1.StatsRequest
	(*StatsResponse)(nil),                        // 25: onebox.faas.vmmd.v1.StatsResponse
	(*InstanceStats)(nil),                        // 26: onebox.faas.vmmd.v1.InstanceStats
	(*PingRequest)(nil),                          // 27: onebox.faas.vmmd.v1.PingRequest
	(*PingResponse)(nil),                         // 28: onebox.faas.vmmd.v1.PingResponse
	(*Header)(nil),                               // 29: onebox.faas.vmmd.v1.Header
	(*HeartbeatRequest)(nil),                     // 30: onebox.faas.vmmd.v1.HeartbeatRequest
	(*HeartbeatResponse)(nil),                    // 31: onebox.faas.vmmd.v1.HeartbeatResponse
	(*UpdateEgressAllowlistRequest)(nil),         // 32: onebox.faas.vmmd.v1.UpdateEgressAllowlistRequest
	(*UpdateEgressAllowlistAck)(nil),             // 33: onebox.faas.vmmd.v1.UpdateEgressAllowlistAck
	(*UpdateEgressCircuitRequest)(nil),           // 34: onebox.faas.vmmd.v1.UpdateEgressCircuitRequest
	(*EgressCircuitTarget)(nil),                  // 35: onebox.faas.vmmd.v1.EgressCircuitTarget
	(*UpdateEgressCircuitAck)(nil),               // 36: onebox.faas.vmmd.v1.UpdateEgressCircuitAck
	(*UpdateStaticEgressIPRequest)(nil),          // 37: onebox.faas.vmmd.v1.UpdateStaticEgressIPRequest
	(*UpdateStaticEgressIPAck)(nil),              // 38: onebox.faas.vmmd.v1.UpdateStaticEgressIPAck
	(*UpdatePrivateNetworkRequest)(nil),          // 39: onebox.faas.vmmd.v1.UpdatePrivateNetworkRequest
	(*UpdatePrivateNetworkAck)(nil),              // 40: onebox.faas.vmmd.v1.UpdatePrivateNetworkAck
	(*SeccompStatusRequest)(nil),                 // 41: onebox.faas.vmmd.v1.SeccompStatusRequest
	(*SeccompStatusResponse)(nil),                // 42: onebox.faas.vmmd.v1.SeccompStatusResponse
	(*LogsRequest)(nil),                          // 43: onebox.faas.vmmd.v1.LogsRequest
	(*LogsResponse)(nil),                         // 44: onebox.faas.vmmd.v1.LogsResponse
	(*MountParentExt4ReadOnlyRequest)(nil),       // 45: onebox.faas.vmmd.v1.MountParentExt4ReadOnlyRequest
	(*MountParentExt4ReadOnlyResponse)(nil),      // 46: onebox.faas.vmmd.v1.MountParentExt4ReadOnlyResponse
	(*MaterializeParentExt4Request)(nil),         // 47: onebox.faas.vmmd.v1.MaterializeParentExt4Request
	(*MaterializeParentExt4Response)(nil),        // 48: onebox.faas.vmmd.v1.MaterializeParentExt4Response
	(*UmountParentExt4Request)(nil),              // 49: onebox.faas.vmmd.v1.UmountParentExt4Request
	(*UmountParentExt4Response)(nil),             // 50: onebox.faas.vmmd.v1.UmountParentExt4Response
	(*MountOverlayParentRequest)(nil),            // 51: onebox.faas.vmmd.v1.MountOverlayParentRequest
	(*MountOverlayParentResponse)(nil),           // 52: onebox.faas.vmmd.v1.MountOverlayParentResponse
	(*UmountOverlayParentRequest)(nil),           // 53: onebox.faas.vmmd.v1.UmountOverlayParentRequest
	(*UmountOverlayParentResponse)(nil),          // 54: onebox.faas.vmmd.v1.UmountOverlayParentResponse
	(*ForwardHTTPStreamRequest)(nil),             // 55: onebox.faas.vmmd.v1.ForwardHTTPStreamRequest
	(*ForwardHTTPRequestInit)(nil),               // 56: onebox.faas.vmmd.v1.ForwardHTTPRequestInit
	(*ForwardHTTPStreamResponse)(nil),            // 57: onebox.faas.vmmd.v1.ForwardHTTPStreamResponse
	(*ForwardHTTPResponseInit)(nil),              // 58: onebox.faas.vmmd.v1.ForwardHTTPResponseInit
	(*PrepareLiveMigrationRequest)(nil),          // 59: onebox.faas.vmmd.v1.PrepareLiveMigrationRequest
	(*PrepareLiveMigrationResponse)(nil),         // 60: onebox.faas.vmmd.v1.PrepareLiveMigrationResponse
	(*AdoptMigratedInstanceRequest)(nil),         // 61: onebox.faas.vmmd.v1.AdoptMigratedInstanceRequest
	(*AdoptMigratedInstanceResponse)(nil),        // 62: onebox.faas.vmmd.v1.AdoptMigratedInstanceResponse
	(*AcknowledgeMigrationRequest)(nil),          // 63: onebox.faas.vmmd.v1.AcknowledgeMigrationRequest
	(*AcknowledgeMigrationResponse)(nil),         // 64: onebox.faas.vmmd.v1.AcknowledgeMigrationResponse
	(*CancelLiveMigrationRequest)(nil),           // 65: onebox.faas.vmmd.v1.CancelLiveMigrationRequest
	(*CancelLiveMigrationResponse)(nil),          // 66: onebox.faas.vmmd.v1.CancelLiveMigrationResponse
	(*ForwardRawRequestInit)(nil),                // 67: onebox.faas.vmmd.v1.ForwardRawRequestInit
	(*ForwardRawRequest)(nil),                    // 68: onebox.faas.vmmd.v1.ForwardRawRequest
	(*ForwardRawResponseInit)(nil),               // 69: onebox.faas.vmmd.v1.ForwardRawResponseInit
	(*ForwardRawResponse)(nil),                   // 70: onebox.faas.vmmd.v1.ForwardRawResponse
	(*WaitBuilderReadyRequest)(nil),              // 71: onebox.faas.vmmd.v1.WaitBuilderReadyRequest
	(*WaitBuilderReadyResponse)(nil),             // 72: onebox.faas.vmmd.v1.WaitBuilderReadyResponse
	(*DeleteWarmSnapshotRequest)(nil),            // 73: onebox.faas.vmmd.v1.DeleteWarmSnapshotRequest
	(*DeleteWarmSnapshotResponse)(nil),           // 74: onebox.faas.vmmd.v1.DeleteWarmSnapshotResponse
	(*ExecuteExecutionRequest)(nil),              // 75: onebox.faas.vmmd.v1.ExecuteExecutionRequest
	(*ExecuteExecutionResponse)(nil),             // 76: onebox.faas.vmmd.v1.ExecuteExecutionResponse
	(*ExecuteExecutionEvent)(nil),                // 77: onebox.faas.vmmd.v1.ExecuteExecutionEvent
	(*ExecuteExecutionOutputChunk)(nil),          // 78: onebox.faas.vmmd.v1.ExecuteExecutionOutputChunk
	(*RestoreExecutionRequest)(nil),              // 79: onebox.faas.vmmd.v1.RestoreExecutionRequest
	(*RestoreExecutionResponse)(nil),             // 80: onebox.faas.vmmd.v1.RestoreExecutionResponse
	(*ReconcilePrivateNetworkFabricRequest)(nil), // 81: onebox.faas.vmmd.v1.ReconcilePrivateNetworkFabricRequest
	(*ReconcilePrivateNetworkFabricAck)(nil),     // 82: onebox.faas.vmmd.v1.ReconcilePrivateNetworkFabricAck
	(*RemovePrivateNetworkFabricRequest)(nil),    // 83: onebox.faas.vmmd.v1.RemovePrivateNetworkFabricRequest
	(*RemovePrivateNetworkFabricAck)(nil),        // 84: onebox.faas.vmmd.v1.RemovePrivateNetworkFabricAck
	(*ForwardTCPRequestInit)(nil),                // 85: onebox.faas.vmmd.v1.ForwardTCPRequestInit
	(*ForwardTCPRequest)(nil),                    // 86: onebox.faas.vmmd.v1.ForwardTCPRequest
	(*ForwardTCPResponseInit)(nil),               // 87: onebox.faas.vmmd.v1.ForwardTCPResponseInit
	(*ForwardTCPResponse)(nil),                   // 88: onebox.faas.vmmd.v1.ForwardTCPResponse
	(*FlowSummary)(nil),                          // 89: onebox.faas.vmmd.v1.FlowSummary
	(*PrivateNetworkFirewallRule)(nil),           // 90: onebox.faas.vmmd.v1.PrivateNetworkFirewallRule
	(*ResumeWarmInstanceRequest)(nil),            // 91: onebox.faas.vmmd.v1.ResumeWarmInstanceRequest
	(*ResumeWarmInstanceResponse)(nil),           // 92: onebox.faas.vmmd.v1.ResumeWarmInstanceResponse
	(*RestoreAppTaskRequest)(nil),                // 93: onebox.faas.vmmd.v1.RestoreAppTaskRequest
	(*RestoreAppTaskResponse)(nil),               // 94: onebox.faas.vmmd.v1.RestoreAppTaskResponse
	(*ExecuteAppTaskRequest)(nil),                // 95: onebox.faas.vmmd.v1.ExecuteAppTaskRequest
	(*ExecuteAppTaskResponse)(nil),               // 96: onebox.faas.vmmd.v1.ExecuteAppTaskResponse
	(*ExecuteAppTaskEvent)(nil),                  // 97: onebox.faas.vmmd.v1.ExecuteAppTaskEvent
	(*ExecuteAppTaskOutputChunk)(nil),            // 98: onebox.faas.vmmd.v1.ExecuteAppTaskOutputChunk
	nil,                                          // 99: onebox.faas.vmmd.v1.JobColdBootRequest.EnvEntry
...
var file_onebox_faas_vmmd_v1_vmmd_proto_depIdxs = []int32{
	...
	90,  // 3: onebox.faas.vmmd.v1.AppSpec.private_network_firewall_rules:type_name -> onebox.faas.vmmd.v1.PrivateNetworkFirewallRule
	3,   // 4: onebox.faas.vmmd.v1.SidecarSpec.sealed_env:type_name -> onebox.faas.vmmd.v1.SealedSecret
	5,   // 5: onebox.faas.vmmd.v1.SidecarSpec.depends_on:type_name -> onebox.faas.vmmd.v1.WorkloadDependency
	0,   // 6: onebox.faas.vmmd.v1.WakeResponse.method:type_name -> onebox.faas.vmmd.v1.WakeMethod
	0,   // 7: onebox.faas.vmmd.v1.WakeResponse.requested_method:type_name -> onebox.faas.vmmd.v1.WakeMethod
	100, // 8: onebox.faas.vmmd.v1.WakeResponse.problem:type_name -> google.protobuf.Struct
	100, // 9: onebox.faas.vmmd.v1.WakeResponse.characterization:type_name -> google.protobuf.Struct
	1,   // 10: onebox.faas.vmmd.v1.CreateFromSnapshotRequest.app:type_name -> onebox.faas.vmmd.v1.AppSpec
	6,   // 11: onebox.faas.vmmd.v1.CreateFromSnapshotRequest.snapshot:type_name -> onebox.faas.vmmd.v1.SnapshotRef
	14,  // 12: onebox.faas.vmmd.v1.CreateFromSnapshotRequest.build:type_name -> onebox.faas.vmmd.v1.BuildSpec
	1,   // 13: onebox.faas.vmmd.v1.CreateColdBootRequest.app:type_name -> onebox.faas.vmmd.v1.AppSpec
	14,  // 14: onebox.faas.vmmd.v1.CreateColdBootRequest.build:type_name -> onebox.faas.vmmd.v1.BuildSpec
	99,  // 15: onebox.faas.vmmd.v1.JobColdBootRequest.env:type_name -> onebox.faas.vmmd.v1.JobColdBootRequest.EnvEntry
	101, // 16: onebox.faas.vmmd.v1.StatsResponse.total_resident_bytes:type_name -> google.protobuf.Int64Value
	26,  // 17: onebox.faas.vmmd.v1.StatsResponse.instances:type_name -> onebox.faas.vmmd.v1.InstanceStats
	101, // 18: onebox.faas.vmmd.v1.InstanceStats.resident_bytes:type_name -> google.protobuf.Int64Value
	102, // 19: onebox.faas.vmmd.v1.InstanceStats.cpu_pct:type_name -> google.protobuf.DoubleValue
	102, // 20: onebox.faas.vmmd.v1.InstanceStats.cpu_seconds:type_name -> google.protobuf.DoubleValue
	102, // 21: onebox.faas.vmmd.v1.InstanceStats.cpu_throttled_seconds:type_name -> google.protobuf.DoubleValue
	103, // 22: onebox.faas.vmmd.v1.InstanceStats.last_request_at:type_name -> google.protobuf.Timestamp
	101, // 23: onebox.faas.vmmd.v1.InstanceStats.net_tx_bytes:type_name -> google.protobuf.Int64Value
	101, // 24: onebox.faas.vmmd.v1.InstanceStats.request_count_total:type_name -> google.protobuf.Int64Value
	101, // 25: onebox.faas.vmmd.v1.InstanceStats.net_rx_bytes:type_name -> google.protobuf.Int64Value
	101, // 26: onebox.faas.vmmd.v1.InstanceStats.disk_used_bytes:type_name -> google.protobuf.Int64Value
	101, // 27: onebox.faas.vmmd.v1.InstanceStats.disk_capacity_bytes:type_name -> google.protobuf.Int64Value
	89,  // 28: onebox.faas.vmmd.v1.InstanceStats.flow_summaries:type_name -> onebox.faas.vmmd.v1.FlowSummary
	103, // 29: onebox.faas.vmmd.v1.PingResponse.server_time:type_name -> google.protobuf.Timestamp
	35,  // 30: onebox.faas.vmmd.v1.UpdateEgressCircuitRequest.circuits:type_name -> onebox.faas.vmmd.v1.EgressCircuitTarget
	90,  // 31: onebox.faas.vmmd.v1.UpdatePrivateNetworkRequest.private_network_firewall_rules:type_name -> onebox.faas.vmmd.v1.PrivateNetworkFirewallRule
	103, // 32: onebox.faas.vmmd.v1.LogsRequest.since_written_at:type_name -> google.protobuf.Timestamp
	103, // 33: onebox.faas.vmmd.v1.LogsResponse.written_at:type_name -> google.protobuf.Timestamp
	103, // 34: onebox.faas.vmmd.v1.LogsResponse.gap_to_written_at:type_name -> google.protobuf.Timestamp
	56,  // 35: onebox.faas.vmmd.v1.ForwardHTTPStreamRequest.init:type_name -> onebox.faas.vmmd.v1.ForwardHTTPRequestInit
	29,  // 36: onebox.faas.vmmd.v1.ForwardHTTPRequestInit.headers:type_name -> onebox.faas.vmmd.v1.Header
	58,  // 37: onebox.faas.vmmd.v1.ForwardHTTPStreamResponse.init:type_name -> onebox.faas.vmmd.v1.ForwardHTTPResponseInit
	29,  // 38: onebox.faas.vmmd.v1.ForwardHTTPResponseInit.headers:type_name -> onebox.faas.vmmd.v1.Header
	29,  // 39: onebox.faas.vmmd.v1.ForwardHTTPResponseInit.trailers:type_name -> onebox.faas.vmmd.v1.Header
	1,   // 40: onebox.faas.vmmd.v1.AdoptMigratedInstanceRequest.app_spec:type_name -> onebox.faas.vmmd.v1.AppSpec
	67,  // 41: onebox.faas.vmmd.v1.ForwardRawRequest.init:type_name -> onebox.faas.vmmd.v1.ForwardRawRequestInit
	29,  // 42: onebox.faas.vmmd.v1.ForwardRawResponseInit.headers:type_name -> onebox.faas.vmmd.v1.Header
	69,  // 43: onebox.faas.vmmd.v1.ForwardRawResponse.init:type_name -> onebox.faas.vmmd.v1.ForwardRawResponseInit
	104, // 44: onebox.faas.vmmd.v1.ExecuteExecutionResponse.exit_code:type_name -> google.protobuf.Int32Value
	78,  // 45: onebox.faas.vmmd.v1.ExecuteExecutionEvent.output:type_name -> onebox.faas.vmmd.v1.ExecuteExecutionOutputChunk
	76,  // 46: onebox.faas.vmmd.v1.ExecuteExecutionEvent.terminal:type_name -> onebox.faas.vmmd.v1.ExecuteExecutionResponse
	6,   // 47: onebox.faas.vmmd.v1.RestoreExecutionRequest.snapshot:type_name -> onebox.faas.vmmd.v1.SnapshotRef
	0,   // 48: onebox.faas.vmmd.v1.RestoreExecutionResponse.method:type_name -> onebox.faas.vmmd.v1.WakeMethod
	0,   // 49: onebox.faas.vmmd.v1.RestoreExecutionResponse.requested_method:type_name -> onebox.faas.vmmd.v1.WakeMethod
	85,  // 50: onebox.faas.vmmd.v1.ForwardTCPRequest.init:type_name -> onebox.faas.vmmd.v1.ForwardTCPRequestInit
	87,  // 51: onebox.faas.vmmd.v1.ForwardTCPResponse.init:type_name -> onebox.faas.vmmd.v1.ForwardTCPResponseInit
	1,   // 52: onebox.faas.vmmd.v1.RestoreAppTaskRequest.app:type_name -> onebox.faas.vmmd.v1.AppSpec
	0,   // 53: onebox.faas.vmmd.v1.RestoreAppTaskResponse.method:type_name -> onebox.faas.vmmd.v1.WakeMethod
	104, // 54: onebox.faas.vmmd.v1.ExecuteAppTaskResponse.exit_code:type_name -> google.protobuf.Int32Value
	98,  // 55: onebox.faas.vmmd.v1.ExecuteAppTaskEvent.output:type_name -> onebox.faas.vmmd.v1.ExecuteAppTaskOutputChunk
	96,  // 56: onebox.faas.vmmd.v1.ExecuteAppTaskEvent.terminal:type_name -> onebox.faas.vmmd.v1.ExecuteAppTaskResponse
	8,   // 57: onebox.faas.vmmd.v1.Vmmd.CreateFromSnapshot:input_type -> onebox.faas.vmmd.v1.CreateFromSnapshotRequest
	9,   // 58: onebox.faas.vmmd.v1.Vmmd.CreateColdBoot:input_type -> onebox.faas.vmmd.v1.CreateColdBootRequest
	10,  // 59: onebox.faas.vmmd.v1.Vmmd.JobColdBoot:input_type -> onebox.faas.vmmd.v1.JobColdBootRequest
	75,  // 60: onebox.faas.vmmd.v1.Vmmd.ExecuteExecution:input_type -> onebox.faas.vmmd.v1.ExecuteExecutionRequest
	75,  // 61: onebox.faas.vmmd.v1.Vmmd.ExecuteExecutionStream:input_type -> onebox.faas.vmmd.v1.ExecuteExecutionRequest
	79,  // 62: onebox.faas.vmmd.v1.Vmmd.RestoreExecution:input_type -> onebox.faas.vmmd.v1.RestoreExecutionRequest
	93,  // 63: onebox.faas.vmmd.v1.Vmmd.RestoreAppTask:input_type -> onebox.faas.vmmd.v1.RestoreAppTaskRequest
	95,  // 64: onebox.faas.vmmd.v1.Vmmd.ExecuteAppTask:input_type -> onebox.faas.vmmd.v1.ExecuteAppTaskRequest
	95,  // 65: onebox.faas.vmmd.v1.Vmmd.ExecuteAppTaskStream:input_type -> onebox.faas.vmmd.v1.ExecuteAppTaskRequest
	12,  // 66: onebox.faas.vmmd.v1.Vmmd.WaitJobExit:input_type -> onebox.faas.vmmd.v1.WaitJobExitRequest
	15,  // 67: onebox.faas.vmmd.v1.Vmmd.PauseAndSnapshot:input_type -> onebox.faas.vmmd.v1.PauseAndSnapshotRequest
	17,  // 68: onebox.faas.vmmd.v1.Vmmd.WarmSnapshot:input_type -> onebox.faas.vmmd.v1.WarmSnapshotRequest
	91,  // 69: onebox.faas.vmmd.v1.Vmmd.ResumeWarmInstance:input_type -> onebox.faas.vmmd.v1.ResumeWarmInstanceRequest
	71,  // 70: onebox.faas.vmmd.v1.Vmmd.WaitBuilderReady:input_type -> onebox.faas.vmmd.v1.WaitBuilderReadyRequest
	73,  // 71: onebox.faas.vmmd.v1.Vmmd.DeleteWarmSnapshot:input_type -> onebox.faas.vmmd.v1.DeleteWarmSnapshotRequest
	18,  // 72: onebox.faas.vmmd.v1.Vmmd.FrameworkReady:input_type -> onebox.faas.vmmd.v1.FrameworkReadyRequest
	20,  // 73: onebox.faas.vmmd.v1.Vmmd.Destroy:input_type -> onebox.faas.vmmd.v1.DestroyRequest
	22,  // 74: onebox.faas.vmmd.v1.Vmmd.StopInstance:input_type -> onebox.faas.vmmd.v1.StopInstanceRequest
	24,  // 75: onebox.faas.vmmd.v1.Vmmd.Stats:input_type -> onebox.faas.vmmd.v1.StatsRequest
	27,  // 76: onebox.faas.vmmd.v1.Vmmd.Ping:input_type -> onebox.faas.vmmd.v1.PingRequest
	30,  // 77: onebox.faas.vmmd.v1.Vmmd.Heartbeat:input_type -> onebox.faas.vmmd.v1.HeartbeatRequest
	32,  // 78: onebox.faas.vmmd.v1.Vmmd.UpdateEgressAllowlist:input_type -> onebox.faas.vmmd.v1.UpdateEgressAllowlistRequest
	37,  // 79: onebox.faas.vmmd.v1.Vmmd.UpdateStaticEgressIP:input_type -> onebox.faas.vmmd.v1.UpdateStaticEgressIPRequest
	34,  // 80: onebox.faas.vmmd.v1.Vmmd.UpdateEgressCircuit:input_type -> onebox.faas.vmmd.v1.UpdateEgressCircuitRequest
	39,  // 81: onebox.faas.vmmd.v1.Vmmd.UpdatePrivateNetwork:input_type -> onebox.faas.vmmd.v1.UpdatePrivateNetworkRequest
	81,  // 82: onebox.faas.vmmd.v1.Vmmd.ReconcilePrivateNetworkFabric:input_type -> onebox.faas.vmmd.v1.ReconcilePrivateNetworkFabricRequest
	83,  // 83: onebox.faas.vmmd.v1.Vmmd.RemovePrivateNetworkFabric:input_type -> onebox.faas.vmmd.v1.RemovePrivateNetworkFabricRequest
	41,  // 84: onebox.faas.vmmd.v1.Vmmd.SeccompStatus:input_type -> onebox.faas.vmmd.v1.SeccompStatusRequest
	43,  // 85: onebox.faas.vmmd.v1.Vmmd.Logs:input_type -> onebox.faas.vmmd.v1.LogsRequest
	55,  // 86: onebox.faas.vmmd.v1.Vmmd.ForwardHTTPStream:input_type -> onebox.faas.vmmd.v1.ForwardHTTPStreamRequest
	68,  // 87: onebox.faas.vmmd.v1.Vmmd.ForwardRawStream:input_type -> onebox.faas.vmmd.v1.ForwardRawRequest
	86,  // 88: onebox.faas.vmmd.v1.Vmmd.ForwardTCPStream:input_type -> onebox.faas.vmmd.v1.ForwardTCPRequest
	45,  // 89: onebox.faas.vmmd.v1.Vmmd.MountParentExt4ReadOnly:input_type -> onebox.faas.vmmd.v1.MountParentExt4ReadOnlyRequest
	47,  // 90: onebox.faas.vmmd.v1.Vmmd.MaterializeParentExt4:input_type -> onebox.faas.vmmd.v1.MaterializeParentExt4Request
	49,  // 91: onebox.faas.vmmd.v1.Vmmd.UmountParentExt4:input_type -> onebox.faas.vmmd.v1.UmountParentExt4Request
	51,  // 92: onebox.faas.vmmd.v1.Vmmd.MountOverlayParent:input_type -> onebox.faas.vmmd.v1.MountOverlayParentRequest
	53,  // 93: onebox.faas.vmmd.v1.Vmmd.UmountOverlayParent:input_type -> onebox.faas.vmmd.v1.UmountOverlayParentRequest
	59,  // 94: onebox.faas.vmmd.v1.Vmmd.PrepareLiveMigration:input_type -> onebox.faas.vmmd.v1.PrepareLiveMigrationRequest
	61,  // 95: onebox.faas.vmmd.v1.Vmmd.AdoptMigratedInstance:input_type -> onebox.faas.vmmd.v1.AdoptMigratedInstanceRequest
	63,  // 96: onebox.faas.vmmd.v1.Vmmd.AcknowledgeMigration:input_type -> onebox.faas.vmmd.v1.AcknowledgeMigrationRequest
	65,  // 97: onebox.faas.vmmd.v1.Vmmd.CancelLiveMigration:input_type -> onebox.faas.vmmd.v1.CancelLiveMigrationRequest
	7,   // 98: onebox.faas.vmmd.v1.Vmmd.CreateFromSnapshot:output_type -> onebox.faas.vmmd.v1.WakeResponse
	7,   // 99: onebox.faas.vmmd.v1.Vmmd.CreateColdBoot:output_type -> onebox.faas.vmmd.v1.WakeResponse
	11,  // 100: onebox.faas.vmmd.v1.Vmmd.JobColdBoot:output_type -> onebox.faas.vmmd.v1.JobColdBootResponse
	76,  // 101: onebox.faas.vmmd.v1.Vmmd.ExecuteExecution:output_type -> onebox.faas.vmmd.v1.ExecuteExecutionResponse
	77,  // 102: onebox.faas.vmmd.v1.Vmmd.ExecuteExecutionStream:output_type -> onebox.faas.vmmd.v1.ExecuteExecutionEvent
	80,  // 103: onebox.faas.vmmd.v1.Vmmd.RestoreExecution:output_type -> onebox.faas.vmmd.v1.RestoreExecutionResponse
	94,  // 104: onebox.faas.vmmd.v1.Vmmd.RestoreAppTask:output_type -> onebox.faas.vmmd.v1.RestoreAppTaskResponse
	96,  // 105: onebox.faas.vmmd.v1.Vmmd.ExecuteAppTask:output_type -> onebox.faas.vmmd.v1.ExecuteAppTaskResponse
	97,  // 106: onebox.faas.vmmd.v1.Vmmd.ExecuteAppTaskStream:output_type -> onebox.faas.vmmd.v1.ExecuteAppTaskEvent
	13,  // 107: onebox.faas.vmmd.v1.Vmmd.WaitJobExit:output_type -> onebox.faas.vmmd.v1.JobExitResponse
	16,  // 108: onebox.faas.vmmd.v1.Vmmd.PauseAndSnapshot:output_type -> onebox.faas.vmmd.v1.SnapshotResponse
	16,  // 109: onebox.faas.vmmd.v1.Vmmd.WarmSnapshot:output_type -> onebox.faas.vmmd.v1.SnapshotResponse
	92,  // 110: onebox.faas.vmmd.v1.Vmmd.ResumeWarmInstance:output_type -> onebox.faas.vmmd.v1.ResumeWarmInstanceResponse
	72,  // 111: onebox.faas.vmmd.v1.Vmmd.WaitBuilderReady:output_type -> onebox.faas.vmmd.v1.WaitBuilderReadyResponse
	74,  // 112: onebox.faas.vmmd.v1.Vmmd.DeleteWarmSnapshot:output_type -> onebox.faas.vmmd.v1.DeleteWarmSnapshotResponse
	19,  // 113: onebox.faas.vmmd.v1.Vmmd.FrameworkReady:output_type -> onebox.faas.vmmd.v1.FrameworkReadyResponse
	21,  // 114: onebox.faas.vmmd.v1.Vmmd.Destroy:output_type -> onebox.faas.vmmd.v1.DestroyResponse
	23,  // 115: onebox.faas.vmmd.v1.Vmmd.StopInstance:output_type -> onebox.faas.vmmd.v1.StopInstanceResponse
	25,  // 116: onebox.faas.vmmd.v1.Vmmd.Stats:output_type -> onebox.faas.vmmd.v1.StatsResponse
	28,  // 117: onebox.faas.vmmd.v1.Vmmd.Ping:output_type -> onebox.faas.vmmd.v1.PingResponse
	31,  // 118: onebox.faas.vmmd.v1.Vmmd.Heartbeat:output_type -> onebox.faas.vmmd.v1.HeartbeatResponse
	33,  // 119: onebox.faas.vmmd.v1.Vmmd.UpdateEgressAllowlist:output_type -> onebox.faas.vmmd.v1.UpdateEgressAllowlistAck
	38,  // 120: onebox.faas.vmmd.v1.Vmmd.UpdateStaticEgressIP:output_type -> onebox.faas.vmmd.v1.UpdateStaticEgressIPAck
	36,  // 121: onebox.faas.vmmd.v1.Vmmd.UpdateEgressCircuit:output_type -> onebox.faas.vmmd.v1.UpdateEgressCircuitAck
	40,  // 122: onebox.faas.vmmd.v1.Vmmd.UpdatePrivateNetwork:output_type -> onebox.faas.vmmd.v1.UpdatePrivateNetworkAck
	82,  // 123: onebox.faas.vmmd.v1.Vmmd.ReconcilePrivateNetworkFabric:output_type -> onebox.faas.vmmd.v1.ReconcilePrivateNetworkFabricAck
	84,  // 124: onebox.faas.vmmd.v1.Vmmd.RemovePrivateNetworkFabric:output_type -> onebox.faas.vmmd.v1.RemovePrivateNetworkFabricAck
	42,  // 125: onebox.faas.vmmd.v1.Vmmd.SeccompStatus:output_type -> onebox.faas.vmmd.v1.SeccompStatusResponse
	44,  // 126: onebox.faas.vmmd.v1.Vmmd.Logs:output_type -> onebox.faas.vmmd.v1.LogsResponse
	57,  // 127: onebox.faas.vmmd.v1.Vmmd.ForwardHTTPStream:output_type -> onebox.faas.vmmd.v1.ForwardHTTPStreamResponse
	70,  // 128: onebox.faas.vmmd.v1.Vmmd.ForwardRawStream:output_type -> onebox.faas.vmmd.v1.ForwardRawResponse
	88,  // 129: onebox.faas.vmmd.v1.Vmmd.ForwardTCPStream:output_type -> onebox.faas.vmmd.v1.ForwardTCPResponse
	46,  // 130: onebox.faas.vmmd.v1.Vmmd.MountParentExt4ReadOnly:output_type -> onebox.faas.vmmd.v1.MountParentExt4ReadOnlyResponse
	48,  // 131: onebox.faas.vmmd.v1.Vmmd.MaterializeParentExt4:output_type -> onebox.faas.vmmd.v1.MaterializeParentExt4Response
	50,  // 132: onebox.faas.vmmd.v1.Vmmd.UmountParentExt4:output_type -> onebox.faas.vmmd.v1.UmountParentExt4Response
	52,  // 133: onebox.faas.vmmd.v1.Vmmd.MountOverlayParent:output_type -> onebox.faas.vmmd.v1.MountOverlayParentResponse
	54,  // 134: onebox.faas.vmmd.v1.Vmmd.UmountOverlayParent:output_type -> onebox.faas.vmmd.v1.UmountOverlayParentResponse
	60,  // 135: onebox.faas.vmmd.v1.Vmmd.PrepareLiveMigration:output_type -> onebox.faas.vmmd.v1.PrepareLiveMigrationResponse
	62,  // 136: onebox.faas.vmmd.v1.Vmmd.AdoptMigratedInstance:output_type -> onebox.faas.vmmd.v1.AdoptMigratedInstanceResponse
	64,  // 137: onebox.faas.vmmd.v1.Vmmd.AcknowledgeMigration:output_type -> onebox.faas.vmmd.v1.AcknowledgeMigrationResponse
	66,  // 138: onebox.faas.vmmd.v1.Vmmd.CancelLiveMigration:output_type -> onebox.faas.vmmd.v1.CancelLiveMigrationResponse
...
func file_onebox_faas_vmmd_v1_vmmd_proto_init() {
	...
	file_onebox_faas_vmmd_v1_vmmd_proto_msgTypes[42].OneofWrappers = []any{}
	file_onebox_faas_vmmd_v1_vmmd_proto_msgTypes[54].OneofWrappers = []any{
		(*ForwardHTTPStreamRequest_Init)(nil),
		(*ForwardHTTPStreamRequest_BodyChunk)(nil),
	}
	file_onebox_faas_vmmd_v1_vmmd_proto_msgTypes[56].OneofWrappers = []any{
