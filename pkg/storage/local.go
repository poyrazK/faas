...
func (l *LocalStorageBackend) LocalPath(key string) (string, bool, error) {
	path, _, ok, err := l.LocalPathWithSource(key)
	return path, ok, err
}

// LocalPathWithSource is LocalPath with stable source classification for
// restore-path telemetry.
func (l *LocalStorageBackend) LocalPathWithSource(key string) (string, LocalPathSource, bool, error) {
	...
	return full, LocalPathSourceBackend, true, nil
}
