...
func TestWakeFanout_Wants(t *testing.T) {
	cases := []struct {
		name                        string
		existing, inFlight, waiting int
		want                        bool
	}{
		{"one wake absorbs a small burst", 0, 1, 50, false},
		{"one wake exactly at its capacity", 0, 1, 80, false},
		{"demand exceeds one wake", 0, 1, 81, true},
		{"demand exceeds two wakes", 0, 2, 161, true},
		{"two wakes still absorb it", 0, 2, 160, false},
		{"existing instances cover demand", 16, 1, 200, false},
		{"existing instances contribute capacity", 2, 1, 240, false},
		{"demand exceeds existing capacity", 2, 1, 241, true},
		{"existing instances at app ceiling", 20, 1, 100000, false},
		{"in-flight wakes at app ceiling", 0, 20, 100000, false},
	}
	for _, tc := range cases {
		f := WakeFanout{MaxInFlight: 20, PerVM: 80, Existing: tc.existing}
		if got := f.wants(tc.inFlight, tc.waiting); got != tc.want {
			t.Errorf("%s: wants(existing=%d, inFlight=%d, waiting=%d) = %v, want %v",
				tc.name, tc.existing, tc.inFlight, tc.waiting, got, tc.want)
		}
	}
}
...
func TestWakeCoord_FansOutWhenDemandExceedsOneInstance(t *testing.T) {
	...
	// Once the first leader reaches the ledger, later callers observe one
	// existing reservation. The coordinator must keep the generation's
	// pre-leader baseline (zero) or it would double-count that wake.
	afterReservation := fanout
	afterReservation.Existing = 1
	// Within the leader's capacity (PerVM=2): follower, not a new wake.
	if _, isLeader, err := coord.Enter(app, afterReservation); err != nil || isLeader {
...
func TestWakeCoord_ExistingCapacityPreventsWarmBurstFanout(t *testing.T) {
	...
	for i := 1; i < 200; i++ {
		if _, isLeader, err := coord.Enter(app, fanout); err != nil || isLeader {
			t.Fatalf("caller %d: leader=%v err=%v, want follower covered by existing capacity", i, isLeader, err)
		}
	}
	if got := len(coord.inflight[app]); got != 1 {
		t.Fatalf("in-flight wakes = %d, want 1; existing instances must prevent leader fan-out", got)
	}
}
...
func TestEngineWakeFanoutForRefreshesExistingFromLedger(t *testing.T) {
	...
	first := e.wakeFanoutFor(context.Background(), app.ID)
	if first.Existing != 0 {
		t.Fatalf("initial Existing = %d, want 0", first.Existing)
	}
