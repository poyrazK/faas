...
// notification; schedd (this file) reacts by routing each row through
// Engine.ReconcileLifecycleInstance, which marks it
// "evicting_account_deleting" and destroys the VM.
//
...
// reconciles every instance owned by the customer. The engine owns
// the state transition, VM destruction, ledger release, and retry
// semantics; this subscriber only supplies the notification wakeup.
...
func (d *DeletionSubscriber) evictAccount(ctx context.Context, accountID string) {
	...
	for _, ins := range rows {
		acted, err := d.engine.ReconcileLifecycleInstance(ctx, ins.ID)
	...
	d.log.Info("schedd: deletion subscriber evicted instances",
		"account", accountID, "observed", len(rows), "evicted", evicted, "errors", len(errs))
}
