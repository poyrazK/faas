...
var EnvContract = []EnvVar{
	...
	{Name: "FAAS_APID_AUTH_SOCKET", Owners: []string{"apid", "gatewayd-public"}, Source: EnvSourceDefault},
	{Name: "FAAS_APID_GITHUBD_BRIDGE_SOCK", Owners: []string{"apid", "githubd"}, Source: EnvSourceDefault},
	{Name: "FAAS_APID_INTERNAL_BASE_URL", Owners: []string{"meterd"}, Source: EnvSourceDefault, Default: "http://127.0.0.1:9101", Note: "Safe Deploy meterd-to-apid operator listener; must remain a loopback HTTP origin"},
	...
	{Name: "FAAS_APID_OTEL_SPANS_WRITER_SOCKET", Owners: []string{"apid", "gatewayd-internal", "gatewayd-public"}, Source: EnvSourceDefault},
	...
	{Name: "FAAS_COMPUTE_MAX_CONCURRENCY", Owners: []string{"vmmd"}, Source: EnvSourceDropin, Note: "host-fact-derived live-instance ceiling installed by node_join"},
	{Name: "FAAS_COMPUTE_MEM_MB", Owners: []string{"vmmd"}, Source: EnvSourceDropin, Note: "host memory reported by node_join"},
	{Name: "FAAS_COMPUTE_VCPUS", Owners: []string{"vmmd"}, Source: EnvSourceDropin, Note: "host vCPU count reported by node_join"},
	{Name: "FAAS_CONSUMER_USAGE_OUTBOX_ROOT", Owners: []string{"gatewayd-internal"}, Source: EnvSourceDefault, Note: "optional durable financial usage spool override; defaults to /var/lib/faas/consumer-usage"},
	...
	{Name: "FAAS_DEPLOYMENT_ID", Owners: []string{"shared"}, Source: EnvSourceGuest, Note: "platform-authored workload identity; injected by the scheduler and gateway, never customer-controlled"},
	{Name: "FAAS_DEPLOYMENT_TAG", Owners: []string{"shared"}, Source: EnvSourceGuest, Note: "platform-authored workload identity; injected by the scheduler and gateway, never customer-controlled"},
	...
	{Name: "FAAS_FLEET_AGE_IDENTITY_PATH", Owners: []string{"apid", "outboundd"}, Source: EnvSourceUnit},
	...
	{Name: "FAAS_GRPC_DEFAULT_DEADLINE", Owners: []string{"shared"}, Source: EnvSourceDefault, Note: "ADR-190; unary gRPC deadline applied when the caller set none (default 60s, 0 disables the bound but keeps the counter)"},
	...
	{Name: "FAAS_JOBS_DISPATCH", Owners: []string{"schedd"}, Source: EnvSourceUnit, Note: "public-beta unit enables queued job dispatch through the vmmd JobColdBoot RPC"},
	...
	{Name: "FAAS_MANAGED_POSTGRES_CANARY_ACCOUNTS", Owners: []string{"shared"}, Source: EnvSourceDefault, Note: "optional comma-separated exact account IDs eligible for managed PostgreSQL staging provisioning; malformed or oversized lists fail closed"},
	...
	{Name: "FAAS_MANAGED_POSTGRES_QUALIFIED", Owners: []string{"shared"}, Source: EnvSourceDefault, Note: "legacy explicit staging-only provider qualification approval; used only when no approval artifact path is configured, and provisioning stays disabled unless true and the remaining qualification gates match"},
	{Name: "FAAS_MANAGED_POSTGRES_QUALIFIED_BACKEND", Owners: []string{"shared"}, Source: EnvSourceDefault, Note: "legacy exact managed PostgreSQL backend ID; used by the fallback env gate when no approval artifact path is configured"},
	{Name: "FAAS_MANAGED_POSTGRES_QUALIFIED_FINGERPRINT", Owners: []string{"shared"}, Source: EnvSourceDefault, Note: "legacy exact non-secret backend fingerprint; used by the fallback env gate when no approval artifact path is configured"},
	{Name: "FAAS_MANAGED_POSTGRES_QUALIFIED_UNTIL", Owners: []string{"shared"}, Source: EnvSourceDefault, Note: "legacy RFC3339 expiry for the staging qualification approval; used by the fallback env gate when no approval artifact path is configured and expired approvals fail closed"},
	...
	{Name: "FAAS_NODE_ID", Owners: []string{"shared"}, Source: EnvSourceGuest, Note: "platform-authored workload identity; injected by the scheduler and gateway, never customer-controlled"},
	...
	{Name: "FAAS_OBJECT_STORAGE_CONFIG", Owners: []string{"apid", "gatewayd-public", "s3-gatewayd", "shared"}, Source: EnvSourceUnit, Note: "gatewayd-public and s3-gatewayd read /etc/faas/object-storage.json; apid uses the same optional drop-in; s3_enabled runtime config separately defaults off"},
	...
	{Name: "FAAS_OCI_PASSWORD", Owners: []string{"shared"}, Source: EnvSourceEnvFile, Note: "read-only runtime credential in /etc/faas/storage.env; imaged and vmmd lifecycle override in /etc/faas/imaged-storage.env"},
	...
	{Name: "FAAS_OTEL_FLUSH_INTERVAL", Owners: []string{"gatewayd-internal", "gatewayd-public"}, Source: EnvSourceDefault},
	{Name: "FAAS_OTEL_SPANS_WRITER_ENABLED", Owners: []string{"apid", "gatewayd-internal", "gatewayd-public"}, Source: EnvSourceDefault},
	...
	{Name: "FAAS_OVERLAY_IP", Owners: []string{"vmmd"}, Source: EnvSourceDefault, Note: "optional local encrypted-overlay IPv4; vmmd can auto-detect it when private-network transport is enabled"},
	...
	{Name: "FAAS_POLAR_EGRESS_BILLING_FROM", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "activation hour; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_EGRESS_BILLING_MODE", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "off (default), shadow, or live; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_EGRESS_METER_ID", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "separate Polar meter UUID; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_EGRESS_MILLICENTS_PER_GIB", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "whole-cent price per GiB; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_EGRESS_USAGE_EVENT_NAME", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "defaults to faas_egress_usage; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_HOBBY_INCLUDED_EGRESS_GIB", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "Hobby monthly allowance; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	...
	{Name: "FAAS_POLAR_OBJECT_STORAGE_BILLING_FROM", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "UTC-month activation boundary; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_OBJECT_STORAGE_BILLING_MODE", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "off (default), shadow, or live; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_OBJECT_STORAGE_METER_ID", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "separate Polar meter UUID summing charge_millicents; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_OBJECT_STORAGE_USAGE_EVENT_NAME", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "defaults to faas_object_storage_usage; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	{Name: "FAAS_POLAR_PRO_INCLUDED_EGRESS_GIB", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "Pro monthly allowance; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	...
	{Name: "FAAS_POLAR_SCALE_INCLUDED_EGRESS_GIB", Owners: []string{"shared"}, Source: EnvSourceSecretsEnv, Note: "Scale monthly allowance; delivered by /etc/faas/secrets/meterd/billing.env and /etc/faas/sealed.env"},
	...
	{Name: "FAAS_PRIVATE_NETWORK_ENABLED", Owners: []string{"schedd", "shared"}, Source: EnvSourceDefault, Note: "dark-launch switch for provider-neutral private-network attachment intent; disabled by default until a connector is deployed"},
	{Name: "FAAS_PRIVATE_NETWORK_FABRIC_ENABLED", Owners: []string{"apid", "schedd", "shared"}, Source: EnvSourceDefault, Note: "dark-launch switch for Gregale-owned private network CRUD and attachment addresses; disabled by default until the fabric is deployed"},
	...
	{Name: "FAAS_REALTIME_MAX_CONNECTIONS", Owners: []string{"realtimed"}, Source: EnvSourceDefault},
	...
	{Name: "FAAS_RESTORE_PREFETCH", Owners: []string{"vmmd"}, Source: EnvSourceDefault, Note: "optional boolean kill switch for the ADR-225 restore working-set prefetch; production default is enabled"},
	...
	{Name: "FAAS_S3_GATEWAY_CONTROL_ADDR", Owners: []string{"s3-gatewayd"}, Source: EnvSourceUnit, Default: "127.0.0.1:9096"},
	{Name: "FAAS_S3_GATEWAY_LISTEN_ADDR", Owners: []string{"s3-gatewayd"}, Source: EnvSourceUnit, Default: "127.0.0.1:8084"},
	{Name: "FAAS_S3_GATEWAY_ROLE", Owners: []string{"s3-gatewayd"}, Source: EnvSourceDropin, Default: "single-box", Note: "production control-plane service must set control-plane explicitly"},
	...
	{Name: "FAAS_SCHEDD_APP_TASK_DISPATCH_CONCURRENCY", Owners: []string{"schedd"}, Source: EnvSourceDefault, Note: "bounded app-task worker pool; 1 by default and at most 32; only consulted when FAAS_APP_TASK_DISPATCH=1"},
	...
	{Name: "FAAS_SECRETS_FILE", Owners: []string{"guest"}, Source: EnvSourceGuest, Note: "guest-init stamps the tmpfs path only for apps opted into secret reload"},
	{Name: "FAAS_SECRETS_RELOAD_ACK_ENDPOINT", Owners: []string{"guest"}, Source: EnvSourceGuest, Note: "guest-init stamps the local metadata endpoint only for apps opted into secret reload; app reports a closed outcome and non-sensitive secret revision"},
	{Name: "FAAS_SECRETS_REVISION_FILE", Owners: []string{"guest"}, Source: EnvSourceGuest, Note: "guest-init stamps a mode-0400 tmpfs file containing only the current non-sensitive secret-set revision for opted-in apps"},
	{Name: "FAAS_SERVICE_CALLER_ASSERTIONS", Owners: []string{"gatewayd-internal"}, Source: EnvSourceDefault, Note: "ADR-206/279 opt-in: mint a signed caller assertion on every internal service call. Workloads can fetch public verification keys from /v1/service-caller-keys; keep this off until the deployment's verifier rollout is ready. A signing failure remains additive and forwards the call unsigned"},
	...
	{Name: "FAAS_STORAGE_FALLBACK_BACKEND", Owners: []string{"imaged", "vmmd", "shared"}, Source: EnvSourceEnvFile, Note: "optional `oci` read/delete fallback during GCS migration; writes remain GCS-only"},
	...
	{Name: "FAAS_TCPD_IDLE_TIMEOUT", Owners: []string{"gatewayd-public"}, Source: EnvSourceDefault, Note: "maximum quiet period for a raw TCP session; defaults to the platform streaming idle timeout"},
	...
	{Name: "FAAS_TCPD_MAX_CONNECTIONS_PER_ACCOUNT", Owners: []string{"gatewayd-public"}, Source: EnvSourceDefault, Note: "optional account-scoped cap for concurrent raw TCP sessions; zero leaves only the process-wide cap"},
	{Name: "FAAS_TCPD_REFRESH_INTERVAL", Owners: []string{"gatewayd-public"}, Source: EnvSourceDefault},
	{Name: "FAAS_TCPD_SCHEDD_TARGET", Owners: []string{"gatewayd-public"}, Source: EnvSourceDefault},
	...
	{Name: "FAAS_TWO_NODE_NODE_A", Owners: []string{"shared"}, Source: EnvSourceDevOnly, Note: "native two-node acceptance fixture; must never be set on a production daemon"},
	{Name: "FAAS_TWO_NODE_NODE_B", Owners: []string{"shared"}, Source: EnvSourceDevOnly, Note: "native two-node acceptance fixture; must never be set on a production daemon"},
	...
	// protection.
	{Name: "FAAS_UPSTREAM_PROBE", Owners: []string{"meterd", "schedd"}, Source: EnvSourceDefault, Note: "off by design until the §9.A rollout gate (spec)"},
	...
	{Name: "FAAS_WORKLOAD_IDENTITY_TTL_SECONDS", Owners: []string{"vmmd"}, Source: EnvSourceDefault, Note: "optional vmmd workload-identity token lifetime override (30..3600 seconds)"},
}
