...
func UnitGatewaydPublic() daemonunit.Unit {
	return daemonunit.Unit{
		...
		// Caddy connections queue in the kernel instead of seeing ECONNREFUSED.
		Requires:              []string{"postgresql.service", "faas-gatewayd-public.socket"},
		...
		Slice:           "faas-cp.slice",
		MemoryMax:       "512M",
		EnvironmentFile: "-/etc/faas/compute-db.env -/etc/faas/secrets/object-storage/provider.env -/etc/faas/otel.env -/etc/faas/tcpd.env",
