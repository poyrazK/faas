...
func TestClientGetRuntimePolicyStatus(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `{"app_id":"app-1","desired_revision":42,"state":"active","coverage":["gateway_app_cache","deployment_traffic"],"serving_gateways":1,"applied_gateways":1,"pending_gateways":0,"stale_gateways":0}`)
	}))
	defer server.Close()
