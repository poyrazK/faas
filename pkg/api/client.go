...
// APIConsumerUsageOptions controls the optional RFC3339 window for the
// consumer usage and usage-quote methods. Empty bounds use the server's
// trailing-30d default.
...
// use GetAPIConsumerUsageQuote to apply the app's versioned rate cards.
...
func (c *Client) GetAPIConsumerUsageQuote(ctx context.Context, slug, consumerID string, opts APIConsumerUsageOptions) (APIConsumerUsageQuoteResponse, error) {
...
func (c *Client) DeployFromSourceTarball(ctx context.Context, slug string, tarball io.Reader, tarballName string, sidecar SourceTarballDeployRequest) (DeploymentResponse, error) {
	...
	if sidecar.Repo != "" || sidecar.Ref != "" || sidecar.Environment != "" || sidecar.Resources != nil || sidecar.MaxInstances != nil || sidecar.Reason != "" || sidecar.Tag != "" ||
...
// UpdateAppSecurity updates the admin-scoped signature flag and deploy-time
// posture policy. Both request fields are optional, so callers can change one
// control without overwriting the other. Audit event: app.security_updated.
