...
type RetryPolicy struct {
	MaxAttempts   int     `json:"max_attempts,omitempty"`   // total attempts incl. the first (1 = no retry)
	BaseSeconds   float64 `json:"base_seconds,omitempty"`   // delay before attempt 1 (typical: 1.0)
	MaxSeconds    float64 `json:"max_seconds,omitempty"`    // cap on the doubled base (typical: 300.0)
	JitterSeconds float64 `json:"jitter_seconds,omitempty"` // symmetric ± fraction of base (0.2 = ±20%)
}
