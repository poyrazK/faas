	...
	if txStore, ok := s.Store.(state.ProjectReconcileStore); ok {
		...
		for _, action := range orderedActions {
			...
			case "update":
				...
				manifest.ServiceBindingPolicy = serviceBindingPolicyForWorkload(action.Workload)
				manifest.PreviewServiceCallsPolicy = previewServiceCallsPolicyForWorkload(action.Workload)
