...
type Config struct {
	...
	// EgressCircuitEnabled emits the ADR-200 §3 egress-circuit set, counter
	...
	// byte-identical output to the pre-ADR-200 renderer — asserted by
...
func (c Config) NftCommands() [][]string {
	...
	// ADR-200 §3: reject NEW connections to an upstream whose circuit is
