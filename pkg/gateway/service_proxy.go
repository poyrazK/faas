...
func (p *ServiceProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if upgrade {
		// Hijacked upgrade responses must receive the original writer. The
		// ordinary trace writer is intentionally limited to buffered HTTP
		// responses and would hide the hijacker interface.
		dispatchWriter = w
	}
	defer func() {
		if !upgrade {
