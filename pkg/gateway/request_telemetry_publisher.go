...
func (k bucketKey) String() string {
	...
	return fmt.Sprintf("%s|%s|%s|%s|%s|%d|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%t|%d|%d|%d",
		...
		k.GuestErrorClass, k.ConsumerID, k.PlatformTenantID, k.PlatformTenantSurfaceID, k.NodeID, k.Region, k.CommitSHA,
