...
func TestFakeSchedulerReturnsAddress(t *testing.T) {
	s := NewFakeScheduler("1.2.3.4:8080")
	addr, err := s.Wake(context.Background(), "app-1")
	if err != nil {
...
func TestFakeSchedulerWithErr(t *testing.T) {
	...
	_, err := s.Wake(context.Background(), "app-1")
...
func TestNoopSchedulerReturnsUnconfigured(t *testing.T) {
	_, err := NoopScheduler{}.Wake(context.Background(), "app-1")
