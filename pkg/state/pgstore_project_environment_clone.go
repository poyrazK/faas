...
func insertClonedProjectEnvironment(ctx context.Context, tx pgx.Tx, clone ProjectEnvironmentClone) (ProjectEnvironment, error) {
	environment := ProjectEnvironment{
		PreviewPRNumber: clone.PreviewPRNumber,
		PreviewHeadSHA:  clone.PreviewHeadSHA,
	}
	if err := initializeProjectEnvironmentPreviewLifecycle(&environment, time.Now().UTC()); err != nil {
		return ProjectEnvironment{}, err
	}
	row := tx.QueryRow(ctx, `
		insert into project_environments
		    (account_id, project_id, slug, protected, preview_pr_number, preview_head_sha, preview_state, preview_expires_at)
	...
	`, clone.AccountID, clone.ProjectID, clone.TargetSlug, clone.TargetProtected,
		clone.PreviewPRNumber, clone.PreviewHeadSHA, environment.PreviewState, environment.PreviewExpiresAt)
...
func copyProjectEnvironmentRows(ctx context.Context, tx pgx.Tx, clone ProjectEnvironmentClone) (ProjectEnvironmentCloneResult, error) {
	...
	if err := tx.QueryRow(ctx, `
		...
		select count(*) from copied
	`, clone.AccountID, clone.ProjectID, clone.SourceSlug, clone.TargetSlug).Scan(&routingCopied); err != nil {
		return result, mapErr(err)
	}
	result.PoliciesCopied += routingCopied
	var ipCopied int
	if err := tx.QueryRow(ctx, `
		with copied as (
		...
		select count(*) from copied
	`, clone.AccountID, clone.ProjectID, clone.SourceSlug, clone.TargetSlug).Scan(&ipCopied); err != nil {
		return result, mapErr(err)
	}
	result.PoliciesCopied += ipCopied
