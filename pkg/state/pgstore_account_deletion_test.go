...
func seedFullAccountWithDep(t *testing.T, s *state.PgStore, ctx context.Context) (string, string, error) {
	...
	if _, err := s.CreateInstance(ctx, app.ID, dep.ID, "running", 256, resolveDefaultLocal(t, ctx, s)); err != nil {
		return "", "", err
	}
...
func TestPg_UsageByAccount_AggregatesByMonth(t *testing.T) {
	...
	app := apps[0]
	ins, err := s.CreateInstance(ctx, app.ID, depID, "running", 256, resolveDefaultLocal(t, ctx, s))
	if err != nil {
