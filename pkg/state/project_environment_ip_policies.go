...
// ValidProjectEnvironmentIPRules also protects gateway reads of tampered rows.
func ValidProjectEnvironmentIPRules(rules []ProjectEnvironmentEdgeRule) bool {
	if len(rules) > 20 {
		return false
	}
	for _, rule := range rules {
		if rule.Kind != EdgeRuleKindIP || rule.Action.Kind != EdgeRuleKindIP || rule.Action.IP == nil ||
			rule.Priority < 0 || rule.Priority > 10000 ||
			!strings.HasPrefix(rule.MatchPath, "/") || len(rule.MatchPath) > 2048 {
			return false
		}
		if _, err := path.Match(rule.MatchPath, "/"); err != nil {
			return false
		}
		if len(rule.Action.IP.Allow) == 0 && len(rule.Action.IP.Deny) == 0 {
			return false
		}
		for _, cidr := range append(append([]string{}, rule.Action.IP.Allow...), rule.Action.IP.Deny...) {
...
func (m *MemStore) GetProjectEnvironmentIPPolicy(_ context.Context, accountID, appID, scope string) (ProjectEnvironmentEdgePolicy, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	app, ok := m.apps[appID]
	...
	policy, ok := m.projectEnvironmentIPPolicies[projectEnvironmentRoutePolicyKey(appID, scope)]
	if !ok {
		return ProjectEnvironmentEdgePolicy{}, ErrNotFound
	}
...
func (m *MemStore) PutProjectEnvironmentIPPolicy(_ context.Context, policy ProjectEnvironmentEdgePolicy) (ProjectEnvironmentEdgePolicy, error) {
	if !ValidProjectEnvironmentIPRules(policy.Rules) {
	...
	now := time.Now().UTC()
	if old, ok := m.projectEnvironmentIPPolicies[key]; ok {
	...
	policy.Rules = cloneProjectEnvironmentEdgeRules(policy.Rules)
	m.projectEnvironmentIPPolicies[key] = policy
	policy.Rules = cloneProjectEnvironmentEdgeRules(policy.Rules)
	return policy, nil
}

func (s *PgStore) GetProjectEnvironmentIPPolicy(ctx context.Context, accountID, appID, scope string) (ProjectEnvironmentEdgePolicy, error) {
	row := s.pool.QueryRow(ctx, `
		select p.account_id, p.project_id, p.app_id, p.environment_slug,
		       p.rules, p.created_at, p.updated_at
		  from project_environment_ip_policies p
...
func (s *PgStore) PutProjectEnvironmentIPPolicy(ctx context.Context, policy ProjectEnvironmentEdgePolicy) (ProjectEnvironmentEdgePolicy, error) {
	if !ValidProjectEnvironmentIPRules(policy.Rules) {
		return ProjectEnvironmentEdgePolicy{}, ErrInvalidArgument
	}
	rules := policy.Rules
	...
	row := s.pool.QueryRow(ctx, `
		insert into project_environment_ip_policies
