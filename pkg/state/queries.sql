...
-- Regression detection query (RequestTelemetryRegression) is deferred
-- to PR-B — the cron-driven regression detector. The CTE-on-CTE shape
-- trips sqlc v1.31's "ambiguous column reference" parser on the
-- (deployment_id) column even after explicit ON clauses; the
-- workaround is a LATERAL join that requires a more invasive
-- query refactor. PR-A ships the per-deployment drilldown query
-- (RequestTelemetryByDeployment) above; the regression detector
-- composes that with the baseline query in Go.
