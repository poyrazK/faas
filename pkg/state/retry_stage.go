...
func retryDeploymentInput(src Deployment, now time.Time) (Deployment, error) {
	...
	var cpuUtilizationTargetPct *float64
	if src.CPUUtilizationTargetPct != nil {
		value := *src.CPUUtilizationTargetPct
		cpuUtilizationTargetPct = &value
	}
	...
	return Deployment{
		...
		OverrideMainDependsOn:   append(json.RawMessage(nil), src.OverrideMainDependsOn...),
