...
// Egress circuit-breaker candidate reads (ADR-197 §3).
...
// ListEgressCircuitCandidates (ADR-197 §3) — MemStore stub, Postgres-only.
//
...
// ADR-197 §3 per-upstream policy. A nil field means "leave unchanged".
...
// UpdateDataUpstreamCircuitBreaker (ADR-197 §3) — MemStore stub,
