...
func TestPg_Jobs_LegacyArtifactVerification(t *testing.T) {
	...
	if _, err := store.JobFinishLegacyArtifactVerification(ctx, job.ID, ref, "node-b", true, ""); !errors.Is(err, state.ErrNotFound) {
	...
	ready, err := store.JobFinishLegacyArtifactVerification(ctx, job.ID, ref, "node-b", true, "")
	if err != nil || ready.ImageMaterializationStatus != "ready" || ready.ImageStorageKey != ref || ready.ImageResolvedDigest != "" {
		t.Fatalf("found artifact = %+v, %v", ready, err)
	}

	if _, err := pool.Exec(ctx,
		`update jobs set image_materialization_status='verifying_legacy', image_materialized_at=null where id=$1::uuid`, job.ID); err != nil {
		t.Fatal(err)
	}
