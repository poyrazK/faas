...
// in production can leave it blank; tests inject a clock.
func NewPusher(store state.Store, stripe StripePusher, log *slog.Logger, now func() time.Time) *Pusher {
	if now == nil {
		now = time.Now
	}
	return &Pusher{store: store, stripe: stripe, log: log, now: now}
}
...
func (p *Pusher) PushHour(ctx context.Context) (int, error) {
	...
	for _, acct := range accounts {
		...
		if err := p.stripe.PushUsageRecord(ctx, acct, start, gb); err != nil {
			p.log.Warn("meter: push usage", "account", acct.ID, "hour", start, "err", err)
			continue
		}
		pushed++
	}
