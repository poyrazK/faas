...
func TestFreeHardStop(t *testing.T) {
	...
	for {
		...
		now = now.Add(time.Minute)
		if err := s.AppendUsage(ctx, acct.ID, app.ID, ins.ID, now, 136*60, 0, 0, 0, 0, 0, 0, 0); err != nil {
...
func TestPaidOverageNoStop(t *testing.T) {
	...
	if err := s.AppendUsage(ctx, acct.ID, app.ID, "inst1", now, 18_432_000*100, 0, 0, 0, 0, 0, 0, 0); err != nil {
...
func TestPaidOverageDedupesPerDay(t *testing.T) {
	...
	if err := s.AppendUsage(ctx, acct.ID, app.ID, "inst1", day1Hour1, 18_432_000*100, 0, 0, 0, 0, 0, 0, 0); err != nil {
		t.Fatalf("seed usage: %v", err)
	}
...
func TestAppendUsagePerInstanceMinute(t *testing.T) {
	...
	if err := s.AppendUsage(ctx, acct.ID, app.ID, "inst-A", m, 100, 1, 0, 0, 0, 0, 0, 0); err != nil {
		t.Fatalf("append 1: %v", err)
	}
	// Redelivered minute: a no-op. mb_seconds stays 100, requests stays 1.
	if err := s.AppendUsage(ctx, acct.ID, app.ID, "inst-A", m, 50, 2, 0, 0, 0, 0, 0, 0); err != nil {
		t.Fatalf("append 2 (redelivered): %v", err)
	}
	// Different minute: separate row.
	if err := s.AppendUsage(ctx, acct.ID, app.ID, "inst-A", m.Add(time.Minute), 75, 1, 0, 0, 0, 0, 0, 0); err != nil {
