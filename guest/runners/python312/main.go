...
func main() {
	...
	addr := ":" + port
	log.Printf("python312 runner: serving on %s (handler=%s, h2c-capable for app_protocol∈{http2,grpc})", addr, *handlerPath)
...
func handle(w http.ResponseWriter, r *http.Request, handlerPath string, signal *internal.RunnerSignal, tailWaitSec int, tailPipePath string) {
	...
	if err != nil {
		internal.ObserveGuestExecution(r.Context(), "python312", started, http.StatusInternalServerError, err).WithProcessUsage(processUsage).ApplyResponseHeaders(w.Header())
	...
	evidence := internal.ObserveGuestExecution(r.Context(), "python312", started, resp.Status, nil).WithProcessUsage(processUsage)
