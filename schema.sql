...
CREATE TABLE public.app_log_drain_health (
    ...
    pending_records integer DEFAULT 0 NOT NULL,
    pending_bytes bigint DEFAULT 0 NOT NULL,
    pending_bytes_capacity bigint DEFAULT 0 NOT NULL,
    dead_letter_total bigint DEFAULT 0 NOT NULL,
    ...
    CONSTRAINT app_log_drain_health_pending_records_chk CHECK ((pending_records >= 0)),
    CONSTRAINT app_log_drain_health_pending_bytes_chk CHECK ((pending_bytes >= 0)),
    CONSTRAINT app_log_drain_health_pending_bytes_capacity_chk CHECK ((pending_bytes_capacity >= 0)),
    CONSTRAINT app_log_drain_health_dead_letter_chk CHECK ((dead_letter_total >= 0)),
...
CREATE TABLE public.deployments (
    ...
    cpu_utilization_target_pct double precision,
    ...
    CONSTRAINT deployments_compute_ram_chk CHECK ((ram_mb >= 0)),
    CONSTRAINT deployments_compute_cpu_chk CHECK ((cpu_millicores = ANY (ARRAY[0, 250, 500, 1000]))),
    CONSTRAINT deployments_max_instances_chk CHECK ((max_instances >= 0)),
    CONSTRAINT deployments_cpu_utilization_target_pct_chk CHECK (((cpu_utilization_target_pct IS NULL) OR ((cpu_utilization_target_pct >= (0)::double precision) AND (cpu_utilization_target_pct <= (100)::double precision)))),
    CONSTRAINT deployments_max_concurrent_requests_chk CHECK (((max_concurrent_requests IS NULL) OR ((max_concurrent_requests >= 1) AND (max_concurrent_requests <= 1000)))),
