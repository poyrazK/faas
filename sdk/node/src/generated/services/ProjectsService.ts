...
import type { UpdateProjectEnvironmentIPPolicyRequest } from '../models/UpdateProjectEnvironmentIPPolicyRequest.js';
...
export class ProjectsService {
   ...
   * An explicit empty list disables inherited IP rules on the stable environment URL. Other rule kinds and ordinary application hosts are unchanged.
   * @returns ProjectEnvironmentEdgePolicyResponse Stored environment-owned IP policy.
   * @throws ApiError
   */
  public static updateProjectEnvironmentIpPolicies({
    slug,
    environment,
    workload,
    requestBody,
    ...
    /**
     * Project containing the environment-specific IP policy.
     */
    slug: string,
    /**
     * Registered environment whose IP rules are replaced.
     */
    environment: string,
    /**
     * Workload slug receiving the scoped IP-rule replacement.
     */
    workload: string,
    requestBody: UpdateProjectEnvironmentIPPolicyRequest,
  }): CancelablePromise<ProjectEnvironmentEdgePolicyResponse> {
    return __request(OpenAPI, {
      method: 'PUT',
      url: '/v1/projects/{slug}/environments/{environment}/workloads/{workload}/ip-policies',
      path: {
        'slug': slug,
        'environment': environment,
        'workload': workload,
