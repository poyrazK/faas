...
func (d *privateNetworkDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	resp.Diagnostics.Append(setPrivateNetworkDataSourceState(ctx, resp, out, networkID)...)
}

func setPrivateNetworkDataSourceState(ctx context.Context, resp *datasource.ReadResponse, out privateNetworkResponse, networkID string) diag.Diagnostics {
	allowedCIDRs, diags := types.SetValueFrom(ctx, types.StringType, out.AllowedCIDRs)
	if diags.HasError() {
		return diags
	}

	state := privateNetworkDataSourceModel{
