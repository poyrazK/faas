...
-- usage_daily's existing app_id-based primary key for compatibility with the
-- v1 read API, but carry an explicit discriminator and the denormalised job
-- identity. The rollup writes app_id=job_id for job rows so the existing
...
COMMENT ON COLUMN usage_daily.meter_kind IS
    'Meter discriminator: app or job. Job rows retain app_id=job_id for the legacy PK; see migration 20260918100000001.';
