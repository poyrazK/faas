...
// Migration-apply test for 00355 (deployment_audit 90-day backfill
// from events — issue #976 / ADR-122 / SAFE-RELEASES-E.2). Pins:
//  1. The migration set applies cleanly through 00355.
//  2. After applying 00355 (against an empty events table) and
...
func TestMigrations_00355_DeploymentAuditBackfill(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply the full migration set through 00355. The backfill
	...
	// Re-execute the same backfill INSERT 00355 ships verbatim. We
	// can't replay 00355 itself (it's already applied); the
