...
// Migration-apply test for 00166_builds_deployment_started_idx.sql
// (DEPLOY-PROV-6 follow-up / ADR-091, issue #741 close-out).
// Pins the keyset-filter index for the new GET /v1/builds
// route:
//
//  1. Migration set applies cleanly through 00166.
...
func TestMigrations_00166_BuildsDeploymentStartedIdx(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Run the full migration set. 00166 lands last on this
	// branch; the apply_walk_test pins contiguity at the directory
	// level.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (ADR-091 slot 166 broken: missing migration slot between 1 and 166)", err)
	}
	...
	// direction.
	slot := "00000000-0000-0000-0000-000000000166"
	...
	if _, err := pool.Exec(ctx, `
		...
		on conflict (id) do nothing
	`, "00000000-0000-0000-0000-000000000166", slot); err != nil {
	...
	if _, err := pool.Exec(ctx, `
		...
		values
		  ('00000000-0000-0000-0000-000000016601', $1, 'railpack', 100, 'queued',  null,                  null),
		  ('00000000-0000-0000-0000-000000016602', $1, 'railpack', 200, 'running', now() - interval '5 minutes', null),
		  ('00000000-0000-0000-0000-000000016603', $1, 'railpack', 300, 'running', now() - interval '1 minute',  null)
		on conflict (id) do nothing
	`, "00000000-0000-0000-0000-000000000166"); err != nil {
	...
	want := []string{
		"00000000-0000-0000-0000-000000016603", // newer running
		"00000000-0000-0000-0000-000000016602", // older running
		"00000000-0000-0000-0000-000000016601", // queued (NULL last)
	}
	if len(got) != len(want) {
	...
	if err := pool.QueryRow(ctx, `
		...
		select id from builds
		 ...
		 order by started_at desc nulls last
		 limit 50
	`, "00000000-0000-0000-0000-000000000166").Scan(&plan); err != nil {
