-- +goose Up
CREATE TABLE outbound_app_bindings (
...
CREATE INDEX outbound_app_bindings_integration_idx
    ON outbound_app_bindings (integration_id, app_id);

-- +goose StatementBegin
CREATE FUNCTION guard_outbound_app_binding_owner() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
          FROM apps app
