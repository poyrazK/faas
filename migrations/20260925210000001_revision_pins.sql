...
CREATE TABLE deployment_revision_pins (
    deployment_id uuid PRIMARY KEY REFERENCES deployments(id) ON DELETE CASCADE,
    app_id uuid NOT NULL REFERENCES apps(id) ON DELETE CASCADE,
    expires_at timestamptz NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX deployment_revision_pins_expiry_idx ON deployment_revision_pins (expires_at);
CREATE INDEX deployment_revision_pins_app_idx ON deployment_revision_pins (app_id, expires_at DESC);

-- +goose Down
DROP TABLE deployment_revision_pins;
