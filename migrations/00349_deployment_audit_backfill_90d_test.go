...
// Migration-apply test for 00349 (deployment_audit 90-day backfill
// from events — issue #976 / ADR-122 / SAFE-RELEASES-E.2). Pins:
//  1. The migration set applies cleanly through 00349.
//  2. After applying 00349 (against an empty events table) and
...
func TestMigrations_00349_DeploymentAuditBackfill(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply the full migration set through 00349. The backfill
	...
	// Re-execute the same backfill INSERT 00349 ships verbatim. We
	// can't replay 00349 itself (it's already applied); the
