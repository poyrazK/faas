...
CREATE TABLE project_release_sets (
...
CREATE UNIQUE INDEX project_release_sets_active_uniq
    ON project_release_sets (project_id, environment_slug) WHERE active;
CREATE INDEX project_release_sets_expiry_idx ON project_release_sets (expires_at);

CREATE TABLE project_release_members (
...
CREATE INDEX project_release_members_deployment_idx
    ON project_release_members (deployment_id, release_id);

-- +goose Down
DROP TABLE project_release_members;
DROP TABLE project_release_sets;
