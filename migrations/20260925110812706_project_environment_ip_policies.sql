...
CREATE TABLE IF NOT EXISTS project_environment_ip_policies (
    ...
    CONSTRAINT project_environment_ip_policies_rules_chk
        CHECK (jsonb_typeof(rules) = 'array' AND jsonb_array_length(rules) <= 20)
);

CREATE INDEX IF NOT EXISTS project_environment_ip_policies_project_idx
    ON project_environment_ip_policies (account_id, project_id, environment_slug);

-- +goose Down
DROP TABLE IF EXISTS project_environment_ip_policies;
