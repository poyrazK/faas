...
APPS_DOMAIN="${FAAS_APPS_DOMAIN:-gregale.dev}"
...
echo "    → install + grant Contents:read + Checks:write + Deployments:write + Issues:write (ADR-012 least-privilege)"
...
echo "  4. least-privilege scopes verified        ✓ (ADR-012: Contents:read + Checks:write + Deployments:write + Issues:write)"
