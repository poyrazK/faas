...
|---|---|---|---|---|---|
| [Circuit breaking](../docs/adr/197-traffic-as-a-platform-primitive) | edge | `internal` | — | Stop routing to an instance that is failing in transport, and probe it back into service once it recovers. | `pkg/gateway/service_proxy_circuit_test.go::TestServiceProxyBreakerReadmitsRecoveredEndpoint` |
...
| [Request retry](../docs/adr/197-traffic-as-a-platform-primitive) | edge | `internal` | — | Replay a request that died in transport against a different healthy instance of the same app. | `pkg/gateway/handler_retry_test.go::TestHandlerRetryConvertsTransportFailureIntoSuccess` |
