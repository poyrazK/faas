...
| `instance_divergence` (`FaasInstanceDivergence`) | warn | [`FaasInstanceDivergence.md`](FaasInstanceDivergence.md) | A telemetry gap spanning two 30 s sweeps on a node that kept reporting other instances; check `schedd_instance_stats_partial_errors_total` for the same node before treating the rows as dead. |
