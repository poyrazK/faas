...
**§4.1.2.15 `kind=cache` — Declarative response cache (ADR-122, ADR-212).** A bounded per-`gatewayd-internal` L1 cache, with an optional Redis-backed shared L2, for safe GET / HEAD responses. The primitive is the `cache` edge-rule kind. Customers declare the route, fresh TTL, optional stale-while-revalidate and stale-if-error windows, and safe `vary_on` dimensions; the gateway owns key construction, purge propagation, background refresh, and cross-instance sharing.
...
- **Stale while revalidate (ADR-212).** `stale_while_revalidate_seconds` is optional and capped at 300 seconds. After freshness expires, the gateway returns the cached response immediately with `x-faas-cache: stale-while-revalidate` and `Warning: 110`, then refreshes the exact key in a detached, singleflight-coalesced request. A zero value disables this behavior.
