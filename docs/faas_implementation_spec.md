...
- **Per-app egress IP allowlist (ADR-031, M8 tier-2):** operators may pin `apps.egress_allowlist cidr[]` on a v4 CIDR list (Pro ≤16 entries, Scale ≤64; Free/Hobby gate). Empty list = current default-allow behaviour preserved. Non-empty list emits a single rule inside the per-netns forward chain — `iifname "tap0" ip daddr { CIDRs… } accept` — placed **after** the lateral-movement deny + SMTP drops so deny > allow on overlap (an operator typo into RFC1918 still gets dropped) and **before** the chain's default policy so unlisted destinations drop. Live instances keep their old ruleset until the next wake (same contract as `RAMMB` / `MaxConcurrency`). v1 is v4 only; v6 mirror is a separate ADR.
