...
**§4.1.2.15 `kind=cache` — Declarative response cache (ADR-122, ADR-208).** A bounded per-`gatewayd-internal` L1 cache, with an optional Redis-backed shared L2, for safe GET / HEAD responses. The primitive is the `cache` edge-rule kind. Customers declare the route, fresh TTL, optional stale-while-revalidate and stale-if-error windows, and safe `vary_on` dimensions; the gateway owns key construction, purge propagation, background refresh, and cross-instance sharing.
