...
| **WB-5** | Operator runbook (`docs/runbooks/twonode-fault-injection.md`) + runbook-as-Go-test (`cmd/e2e/twonode_runbook_test.go`); two-node harness (`pkg/e2etest/twonode.go`) + fault injector (`pkg/e2etest/fault.go`). | `TestRunbook_Drill{1,2,3}` pass under `make metal-lima-2node-fault`; metal acceptance at `cmd/e2e/twonode_failure_safe_metal_test.go`. |
