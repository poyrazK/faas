...
| Free first-week defaults (env vars / secrets / idle / per-VM concurrency / log archive) | 16 / 8 / 60 s / 4 / 1 day | `apid` plan limits, gateway archive read-back, and the customer-facing CLI/dashboard quota surfaces |
| Plan per-VM concurrency bound (`concurrency_per_vm`) | Free 4 · Hobby 5 · Pro 25 · Scale 80 | `apid` validation on `GET /v1/apps/{slug}`; CLI `gregale app <slug> --concurrency`; **listener-level only** — handler-process concurrency is the customer's responsibility (§4.9.1, issue #559) |
...
```
                        ...
                        │                                                                              │
  client ── TLS ──►  Caddy / Cloudflare ──► gatewayd-public ──► gatewayd-internal ──── route lookup (cache→PG) ────┐ │
  *.gregale.dev              │                                      │                                       │
   ...
   deploy (API/CLI) ──► apid ───────┤                                                                  │
                        ...
                        └─► builderd ── ephemeral builder microVM (Railpack/BuildKit inside)           │
                                 ...
                                 imaged ── OCI ➜ ext4 rootfs + guest-init ➜ boot ➜ snapshot            │
                                    │                                                                  │
                        meterd ── 1 s samples ➜ minute rows ➜ GB-h ➜ provider usage records            │
                        └──────────────────────────────────────────────────────────────────────────────┘
   off-node: object storage (build cache, cold snapshots, backups) · off-host PG WAL destination · billing provider · DNS
```

**Request path (hot):** upstream TLS termination (Caddy/Cloudflare) → plain-HTTP
`gatewayd-public` → `gatewayd-internal` → routing cache hit → proxy to instance
IP:8080 → response. Budget: < 2 ms added latency inside Gregale.

**Request path (cold wake):** `gatewayd-public` receives the already-decrypted
request from the upstream edge and hands it to `gatewayd-internal`, which sees
the app has no running instance → holds the request → asks `schedd` → admission
check (RAM headroom, plan concurrency) → `vmmd` restores the snapshot into a
fresh netns/TAP → guest resumes (app already initialized in snapshot memory) →
readiness ping → proxy. The platform-only snapshot wake gate is p95 < 350 ms
from capacity admission/boot start through the first upstream byte (§6.3).
Cloudflare, public network, and client distance remain separate end-to-end costs.
...
| 007 | Edge: **`gatewayd-public` (plain-HTTP ingress) and `gatewayd-internal` (routing/wake/proxy) are our own Go binaries** behind the upstream Caddy/Cloudflare TLS boundary | Wake-blocking (hold request during restore) is core product logic — we own it; the upstream edge owns certificate automation | Making the upstream proxy responsible for wake coordination (split brain), nginx+lua (unmaintainable) |
...
| 010 | Billing: **Polar subscriptions + metered overage item by default**; usage pushed hourly via the provider-neutral `billing.Provider` interface (ADR-025 + ADR-032 v2) | Polar is the public-release merchant of record; the adapter interface (`pkg/billing/Provider`) keeps the runtime provider-neutral, with Paddle and Stripe retained only as explicit compatibility deployments. Dunning states in §10 | Homegrown invoicing (no), provider-specific calls from `meterd` (no) |
...
**Owns:** public plain-HTTP ingress from the upstream edge (`gatewayd-public`),
and routing + wake-blocking + request accounting + per-tenant rate limits
(`gatewayd-internal`). TLS termination and certificate automation are owned by
the upstream Caddy/Cloudflare edge.

- Listeners: the plain-HTTP public socket and the node-local unix socket to
  `gatewayd-internal`. The upstream Caddy/Cloudflare edge owns `:443`/`:80`
  and forwards only trusted traffic to `gatewayd-public`.
...
`gatewayd-public` is the **only public listener on the box**; it receives
...
| `/docs`, `/docs/...`                   | Anonymous Swagger UI API reference                        | Human-readable API discovery                    |
...
- **External-`$ref` strip.** apid-Validate strips external `$ref` / `$id` URLs at create-time (the regex at `dto.go::edgeRuleValidateRefURLPattern`); `pkg/edgevalidate.Compile` re-strips at compile time on the gateway side. Internal JSON Pointers (`#/definitions/Foo`) and relative refs (`../schemas/common.json`) pass both regexes: the apid-side pattern was anchored to `"$ref"`/`"$id"` as a JSON key followed by a URL-shaped value (issue #850; the pre-fix `$ref|id` alternation was unanchored and wrongly rejected any schema containing the substring `id`, e.g. `definitions`). The gateway-side regex remains the authoritative gate on the hot path. A runtime `$ref` URL in a stored rule emits 502 `bad_gateway` from the gateway compile path (handler.go applyEdgeRuleValidate line ~1649) — a deploy-time bug-class alarm.
...
1. `slugFor(host)` — production subdomain (`{slug}.gregale.dev`).
...
5. Preview parser — `pr-{N}-{slug}.gregale.dev` (ADR-095 / PR-B #872).
...
- **Cert engine (feature-flagged tenant-surface path).** This optional issuer is
  separate from production public wildcard TLS, which terminates at the
  upstream Caddy/Cloudflare edge. When tenant surfaces are enabled, a surface
  mints one cert per `cert_kind` value. `per_host_san` (v1 default) bundles up
  to 100 hostnames as Subject Alternative Names against one LE order; `per_host`
  (fallback) mints one cert per hostname; `shared_wildcard` (deferred follow-up
  ADR) is reserved in the schema but the `CertIssuer` fails closed with
  `ErrUnsupportedCertKind` until the DNS-01 solver ships. The
  `CertIssuer.RequestCertForSurface(ctx, surfaceID)` re-mint is synchronous on
  every surface mutation (create, hostname add, hostname remove) — human-paced
  re-mint frequency, bounded by surface evolution. The allowlist
  (`pkg/gateway/allowlist.go:90-156`) gains a surface branch in `OnDemandLookup`
  that fails closed if the surface row is unverified; the on-demand
  `OnDemandAllowlist` signature stays binary in v1 because the per-mutation
  re-mint path handles the SAN group out-of-band.
...
- **PR-preview coexistence.** The `pr-{N}-{slug}.gregale.dev` shape (ADR-095 PR-B, `pkg/gateway/preview_parser.go`) is a *fourth* lookup branch in `pgRouter.ResolveHost`, AFTER `SurfaceByHostname`. Ordering matters: a SaaS customer's preview app should not be visible to a surface-bound FQDN, and a SaaS customer's surface should not be visible as a preview. Both branches are gated by `*(host)` shape-match before any DB call, so the common case (`{slug}.gregale.dev` or `api.customer-a.com`) hits only one branch.
...
- **D4. Per-endpoint RESPONSE cap.** Mirrors the §4.1.2.13 per-rule REQUEST cap. A kind=limit rule with `max_body_bytes_streaming > 0` overrides the plan-level streaming cap (`pkg/api/Plan.MaxResponseBodyBytes()`) on the response side. The DTO `s ≥ b` invariant (`pkg/api/dto.go`) is enforced by `EdgeRuleLimitAction.Validate` on both edge-rule create and update before persistence; the gateway compile-time clamp remains defense-in-depth for direct database writes. `CapKind` label is `"plan"` or `"endpoint-rule"`; stamped on the audit payload + the SDK probe response.
- **D5. CreateApp 403 (Free + flag).** `cmd/apid/handlers.go::buildApp` rejects `streaming_enabled=true` on Free plans with 403 `CodePlanStreamingNotAllowed` — mirroring the existing UpdateApp gate at `cmd/apid/handlers_ext.go:245-252`. Defense-in-depth mirror in `decideStreaming` (plan-disallows branch) catches any pre-D5 Free + flag row that survives the data-tier migration. The database backstop is now shipped by `20260922131114370_apps_streaming_plan_check.sql`: a validated CHECK rejects Free app opt-ins and a constraint trigger rejects paid → Free account downgrades while an app remains opted in.
- **D6. SDK + CLI probe.** `GET /v1/apps/{slug}/streaming-cap` returns `AppStreamingStatus{AppID, Status, EffectiveCap, PlanCap, FlagEnabled, PlanAllowed, CapKind}`. Auth chain: read-only, no MFA, primary caller is an API key with `ScopesReadSurface`. IDOR-safe via loadApp (cross-account slug → 404). The apid-side mirror of `decideStreaming` evaluates the four conjuncts it can read from the apid cache (per-app flag, plan tier, upgrade header, Accept header). Route-aware callers may supply `host`, `path`, and `method` together; apid then asks gatewayd's loopback control listener for the compiled per-edge-rule response cap, safely falling back to the plan cap when no match or gatewayd failure occurs. CLI mirror: `gregale apps streaming-cap <slug>` and `gregale app <slug> streaming-cap`. SDK mirrors: `pkg/api.GetAppStreamingStatus` and `GetAppStreamingStatusForRequest` (hand-written Go), auto-regenerated Node + Python via `make sdk-gen-twice`.
...
**§4.1.2.15 `kind=cache` — Declarative response cache (ADR-122, ADR-214).** A bounded per-`gatewayd-internal` L1 cache, with an optional Redis-backed shared L2, for safe GET / HEAD responses. The primitive is the `cache` edge-rule kind. Customers declare the route, fresh TTL, optional stale-while-revalidate and stale-if-error windows, and safe `vary_on` dimensions; the gateway owns key construction, purge propagation, background refresh, and cross-instance sharing.
...
- **Store: local L1 plus optional distributed L2.** `sync.Mutex` + `container/list` LRU + byte ceiling forms the always-available 64 MiB per-gateway L1. Setting `FAAS_GATEWAY_RESPONSE_CACHE_REDIS_URL` attaches a TTL-bound Redis L2 shared by gateway instances. Redis is never authoritative: connection or operation failure falls through to L1/origin, and no cache error may fail a request. Entries are not persisted to Postgres or gateway disks. App/rule/deploy invalidations and `gregale cache purge` evict both tiers.
- **Stale while revalidate (ADR-214).** `stale_while_revalidate_seconds` is optional and capped at 300 seconds. After freshness expires, the gateway returns the cached response immediately with `x-faas-cache: stale-while-revalidate` and `Warning: 110`, then refreshes the exact key in a detached, singleflight-coalesced request. A zero value disables this behavior.
...
- **Saved-cost surface (telemetry-only, NOT a new SKU).** `gateway_response_cache_total{outcome}` remains the global operator counter, closed at `{hit, miss, bypass_authed, bypass_uncacheable, stale_while_revalidate_served, stale_if_error_served, store_skipped}` (pre-instantiated). The additive customer-facing `gateway_response_cache_app_total{app,outcome}` counter uses the same closed outcomes; customer hit rate is strictly `hit / (hit + miss)`, excluding bypass and stale outcomes. `gateway_response_cache_wakes_avoided_total{app}` increments ONLY when a fresh hit lands on an app with zero healthy instances (`h.backend.HealthyCount(appID) == 0`). `gateway_response_cache_bytes` / `gateway_response_cache_entries` measure the local L1 only. Saved cost remains reporting-layer telemetry; `gb_ram_hour` is unchanged.
...
- **Stale while waking (issue #1395 A1).** When a request arrives while another request is already driving the app's wake gate, an eligible stale entry is served immediately with `x-faas-cache: stale-while-waking`. The wake leader continues to the restored instance and its normal cache writer refreshes the entry; `gateway_cache_stale_while_waking_total{app}` records the follower serve without inflating the fresh-hit rate.
...
- **Idle reaper:** every 10 s, park instances with `now − last_request_at > idle_timeout(plan)`. Defaults: Free 60 s, Hobby 60 s, Pro 300 s, Scale 600 s (app-configurable down to 10 s, not above plan default × 2).
...
**Owns:** usage truth. Sampling → aggregation → quota state → the selected `billing.Provider` (Polar for public release; ADR-032 v2).

- **Canary mirror gate (issue #1395, B1):** a custom canary stage may declare `mirror_clean: {min_invocations, window_s}`. `meterd` only advances that stage after the enabled traffic-mirror summaries for the deployment report at least `min_invocations` in the window and zero status, schema, body, or crash differences. Any difference aborts through `/rollouts/recover` with the measured counts in `RolloutAbortedReason`; the operator does not need to run `gregale canary advance` for a clean stage.

- Sample loop (1 s): for each RUNNING instance read cgroup `memory.current` (host truth, includes VMM) → accumulate `mb_seconds`. Flush app rows to `usage_minutes(account_id, app_id, instance_id, minute, mb_seconds, requests, cpu_usec, tx_bytes, net_tx_bytes)`; job-task rows use the migration-00257 widening (`app_id=NULL, job_id=<jobs.id>, meter_kind='job'`). The daily rollup retains `app_id=<job id>` for its legacy primary key and records `meter_kind='job'`. `cpu_usec` is the cumulative host-cgroup CPU-µs delta (ADR-039, additive on `(instance_id, minute)`); `tx_bytes` is the cumulative HTTP response bytes the gateway forwarded for this instance in this minute (ADR-046, additive); `net_tx_bytes` is the cumulative byte delta on root-side `vethHost.rx_bytes` for this instance in this minute (ADR-046, additive). The first three columns are billable-floor or telemetry; `tx_bytes` and `net_tx_bytes` are telemetry only — no provider push in this PR. On the streaming path (ADR-047), `tx_bytes` is attributed per-flush via `statusRecorder.doFlush`'s `onFlush` callback (the residual after the last flush boundary is captured at `finalFlush` so a response that ends mid-chunk is not silently lost).
...
- Quota ladder per account per month: 0–100 % of included GB-h: nothing; 100 %: email; Free tier at 100 %: hard stop (park, don't wake, `402` page); paid tiers: overage accrues at €0.01/GB-h, pushed hourly as provider usage records through the adapter interface.
- Provider catalog: one product and monthly price per plan, one metered price (`gb_ram_hour`), and one customer plus subscription per account. The public-release provider is Polar; Paddle and Stripe webhooks remain compatibility paths and are selected explicitly with `FAAS_BILLING_PROVIDER`.
- Dunning: `payment_failed` → account `past_due` (apps run, deploys blocked) → 7 days → `suspended` (apps parked, wake returns `402` page) → 21 days → `deleted_pending` (30-day snapshot retention, then GC). All transitions emailed. Subscription end (`subscription.revoked` / provider equivalent) → plan `free` + subscription binding cleared, status untouched: a voluntary cancel stays `active` (one "subscription ended" email), a non-payment revocation keeps its dunning stamp. Free → paid is a hosted-checkout hand-off — `PATCH /v1/account/plan` 402 `checkout_url`, `/dashboard/upgrade`, `gregale plan` — and the plan flips only on the provider webhook.
...
- **`POST /v1/invoices/{id}/consume-credits`** (issue #279 PR-C; admin scope + `FAAS_ADMIN_EMAILS` allowlist + MFA-gated, idempotent via the `Idempotency-Key` header) is the operator trigger for the credit consumption reducer. The reducer reads `usage_minutes.mb_seconds` for the invoice's billing period `[period_start, period_end)`, converts to integer cents via `floor(mb_seconds × 100 / 3600)` at `OverageMillicentsPerGBHour` (€0.01/GB-h, floored; same formula as `pgstore.CurrentMonthOverageCents`), and drains the account's active credits FIFO (`created_at ASC`) against the overage. `cents_remaining` never goes negative: each credit decrement is `UPDATE … WHERE cents_remaining >= $amt RETURNING cents_remaining`, conditional inside a single PgStore transaction. The `consume_credits` loop is the same reducer that a future `meterd` cron and the PR-B `UpsertInvoice` webhook Tx both call — the HTTP endpoint is just one trigger. Idempotent under webhook redelivery via the `UNIQUE (provider, provider_invoice_id, credit_id) WHERE provider_invoice_id IS NOT NULL AND delta_cents < 0` partial index on `credit_ledger` (ADR-220); a second reducer call for the same account, provider and `provider_invoice_id` returns `already_consumed_for_invoice=true` with the same `consumed_cents` and writes no new ledger rows. Migration `00058` adds the invoice ID and initial partial unique index; ADR-220 and migration `20260922183947369` qualify the receipt by provider and fail closed on legacy rows whose provider cannot be resolved safely. Issuance rows leave `provider_invoice_id` NULL so the partial constraint does not apply.
...
- **Provider refund seam**: `billing.Provider.Refund(ctx, chargeID, amountCents)` is on the interface. The operator route `POST /v1/admin/accounts/{id}/refunds` currently supports Polar order IDs, binds the provider order to a paid local invoice, requires MFA for session principals plus the admin allowlist, and forwards the explicit `Idempotency-Key` through the provider context. Polar ignores the `Idempotency-Key` request header, so the Polar adapter stamps the key into refund metadata, lists the order's refunds before writing, sends the refund POST exactly once (no transport retry), and re-lists on an ambiguous failure to recover a committed refund — a redelivered operator request with the same key never moves money twice. Stripe and Paddle retain their adapter implementations for compatibility deployments; a provider without `CapRefund` returns 501. Provider refund webhooks remain observational and emit a `refund.processed` audit row after verification.
...
create table apps (
  id uuid primary key default gen_random_uuid(),
  account_id uuid not null references accounts(id),
  slug text unique not null,                    -- {slug}.gregale.dev
...
| `prewarm.scheduled` | apid `handlers_prewarm.go::createPrewarm` | `{intent_id, app_id, count, wake_at, expires_at, trigger}` — emitted after the durable intent is accepted. Subject is the owning account id. |
| `prewarm.fired` | schedd `pkg/sched/prewarm/trigger.go` | `{intent_id, app_id, count, admitted_count, wake_at, expires_at, status, outcome}` — emitted after a claim is admitted, partially admitted, or fails. Subject is the owning account id when present; system-level producers may omit it. |
...
- **`credit.consumed`** — emitted by `apid` on `POST /v1/invoices/{id}/consume-credits` (issue #279 PR-C); future callers (`meterd` cron, PR-B `UpsertInvoice` webhook Tx) reuse the same reducer and stamp the same audit kind with their own `actor` string. `subject` = the beneficiary account id (the account whose credits were drained). `actor` = `"apid"` for the admin endpoint today. `data` payload: `{credit_id, delta_cents, invoice_id, provider_invoice_id, period_end, total_consumed_cents_for_invoice, remaining_credits_cents}`. One row per drained credit (FIFO order). Idempotent under webhook redelivery via the partial unique index on `credit_ledger (provider, provider_invoice_id, credit_id)` — a second reducer call writes no new ledger rows and no new audit rows; the response returns `already_consumed_for_invoice=true` instead.
...
budget. Runs on `make test-metal` on the supported native x86_64 Linux KVM
acceptance host.
...
- Public: the upstream Caddy/Cloudflare edge owns :80/:443; `gatewayd-public`
  is the only Gregale listener exposed behind that trusted boundary. Nothing
...
- DNS names: `{slug}.gregale.dev` resolves through the upstream edge to the
  serving node. Custom domains point at the edge; `apid` verifies ownership
  via TXT `_faas-verify.{domain}` before the edge routes the name. **Doctor
  (ADR-120):** `GET /v1/domains/{domain}/doctor` returns the 5-check
  Render-style report (DNS found / points to Gregale / TLS / CAA permits /
  IPv6 conflict) with per-check remediation lines; backed by the persisted
  `domain_doctor_observations` table the `dns_poller` writes every 30s. See
  [`docs/domains/doctor.md`](../domains/doctor.md).
...
- Included quotas per plan per calendar month (UTC): 5 / 50 / 250 / 1,500 GB-h. Overage €0.01/GB-h, metered in millicents, provider usage records hourly, idempotent by `(subscription_item, hour)`.
...
- Plan changes: upgrade immediate with provider-managed proration; downgrade at period end; quota checks (deployed count, RAM sizes) run pre-downgrade and block with a task list ("delete 3 apps or reduce RAM…").
...
**Response headers (issue #249):** every response leaving the upstream edge → `gatewayd-public` path and the apid loopback listener carries the six hardening headers below. Mounted by `pkg/httpsec` at the outermost wrapper of both daemons. The static set (HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy) runs on every response; Content-Security-Policy is gated so it only fires on apid-handled URLs (customer-app responses keep the customer's own CSP).
...
**Metrics-listener canonical shape — post-merge audit (ADR-122 amendment):** the audit that confirmed the canonical shape PR also surfaced four production HTTP listeners with a partial knob set. Each is now closed: (a) `cmd/gatewayd-public/main.go::buildServers` **publicSrv** (customer-facing plain-HTTP ingress behind the upstream TLS edge) — `IdleTimeout=120s`; RT/WT use `CustomerRequestEnvelopeTimeout` so the largest supported upload can complete before guest execution. (b) `cmd/gatewayd-public/main.go::buildServers` **controlSrv** (loopback :9092 healthz/readyz/metrics) — adopts the canonical metrics variant (RHT=10s + RT=10s + WT=10s + IT=60s + MHB=1 MiB), RHT bumped from 5s to 10s. (c) `pkg/gateway/control.go::NewControlHTTPServer` (the helper behind `RunControlServer`) — adds `MaxHeaderBytes=1 MiB` (was 0, stdlib default); the four timeout knobs (RHT=5s + RT=10s + WT=10s + IT=60s) pre-date this PR. The helper is exported so a test can pin the struct fields without binding a real listener (mirrors `pkg/githubd.NewWebhookHTTPServer`). (d) `pkg/gateway/synth.go::NewSynthServer` (SynthServer — internal compute loopback) — retains the canonical shape until `SetHandler` installs the unified customer mux, at which point RT/WT use `CustomerRequestEnvelopeTimeout`; RHT=5s stays as the H2C negotiation guard. Intentional exceptions: `cmd/vmmd-stream-bridge/main.go` (per-guest H2C, session-level deadline) and `cmd/gregale/templates/hello-go/main.go` (sample guest template, not production). Full rationale in §"Post-merge audit (issue #995 closure)" of ADR-122.
...
| `Content-Security-Policy` | `default-src 'self'; script-src 'self' 'nonce-{rand}' https://unpkg.com; style-src 'self' 'nonce-{rand}'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self' https://polar.sh` | Per-request 128-bit nonce (base64-URL, 22 chars). Minted by `pkg/httpsec.Nonce`; consumed by `pkg/dashboard.Render` which stamps `nonce="…"` on every `<script>` and `<style>` tag. `https://unpkg.com` is on the `script-src` list because every dashboard template loads `htmx.org@2.0.4` (and SSE pages additionally load `htmx-ext-sse@2.2.2`) from there. SRI hashes pending as a separate ADR. |
...
`docs/runbooks/active-passive-ha.md`; the native two-node acceptance harness
is documented in `docs/ops/native-m9-acceptance.md`. Probe timeout bounded by
...
| `gateway_response_cache_total{outcome}` rate | n/a (per-outcome) | global operator surface; `outcome="bypass_authed" > 5 % / 5 m` warn; `outcome="bypass_uncacheable"` is informational; `outcome="store_skipped"` rate > 50 % warn; `outcome="stale_while_revalidate_served"` reports intentional SWR serves; `outcome="stale_if_error_served"` rate > 5 % / 5 m warns that the origin is flapping (runbook `FaasResponseCache.md`) |
| `gateway_response_cache_app_total{app,outcome}` rate | n/a (per-app) | customer hit rate is `outcome="hit" / (outcome="hit" + outcome="miss")`; bypass and stale outcomes are excluded from the ratio, and absence means no cache telemetry for the app/window |
| `gateway_response_cache_wakes_avoided_total{app}` rate | n/a (per-app) | counter, NOT a rate — dashboards compute `saved_gb_ram_hours = wakes_avoided × (plan.RAM_MB + 8) × billed_seconds / 3600 / 1024` against `pkg/api/limits.go` plan table; telemetry only, does NOT enter `gb_ram_hour` push (ADR-122 §Decision — honest saved-cost figure requires `HealthyCount == 0` gate; a hit against a warm app saves latency but no compute and must not be counted) |
| `gateway_cache_stale_while_waking_total{app}` rate | n/a (per-app) | stale cache responses served while the app wake gate is in progress; informational, separate from fresh-hit and stale-on-error outcomes (issue #1395 A1) |
| `gateway_response_cache_bytes` / `gateway_response_cache_entries` gauge | ≤ `DefaultResponseCacheMaxBytes` (64 MiB) / ≤ entries that fit the byte ceiling | local-L1 gauge high-water alert runs at 80 % of `DefaultResponseCacheMaxBytes` (runbook `FaasResponseCache.md` — sustained pressure means LRU is evicting faster than the dashboard's hit-rate numerator can settle; consider shrinking `vary_on` cardinality) |
...
| `vmmd_snapshot_materialize_seconds{artifact,source}` p95 | `source="materialized"` rare; local sources ≪ 10 ms | `histogram_quantile(0.95, sum by (le) (rate(vmmd_snapshot_materialize_seconds_bucket{artifact="mem",source="materialized"}[5m]))) > 0.2` for 15 m warn — wakes are fetching the memory blob across the network. At the 130 MB fleet snapshot target one fetch can exceed the whole §6.3 wake budget, so a sustained rate means placement is landing away from snapshot replicas (ADR-063). Compare against the local-source quantile to price locality |
| `vmmd_snapshot_materialize_bytes_total{artifact,source}` rate | n/a | none — diagnostic. Divided by the histogram it gives effective fetch throughput, which separates "the object is large" from "the fetch was slow". The `source="materialized"` series is the fleet's remote snapshot read volume |
| `<daemon>_db_notify_hub_delivered_total{channel}` rate | n/a | none — diagnostic. `pg_notify` carries no routing, so one emit fans out to every interested subscriber on every daemon on every node. `sum by (channel) (rate(...))` divided by the channel's emit rate is the **broadcast amplification factor**; it grows linearly with fleet size while useful work stays flat |
| `schedd_app_ownership_checks_total{outcome}` rate | `not_owned` ≈ (N-1)/N of the total on an N-node fleet | none — diagnostic, and the consumer-side half of the above. ADR-062 shards apps by `apps.node_id`; every schedd still receives every app-scoped notification and all but the owner discard it at `ownsApp`. A `not_owned` share materially **above** (N-1)/N means ownership is skewed, not merely broadcast. Read both series before deciding whether per-owner notify channels are worth building |
| `<daemon>_db_pool_conns{state}` gauge | ≪ `<daemon>_db_pool_max_conns` | `sum by (daemon) (<daemon>_db_pool_conns) / <daemon>_db_pool_max_conns > 0.9` for 10 m warn — the daemon is running against the cap in `pkg/db.DaemonMaxConnections`, which is the per-daemon input the `postgres_capacity` role multiplies across the fleet to derive `max_connections` |
| `<daemon>_db_pool_canceled_acquires_total` rate | 0 | > 0 / 5 m page (`FaasDBPoolStarved`, runbook `FaasDBPoolStarved.md`) — a caller abandoned an acquisition while waiting for a pool connection. Unlike the empty-acquire counter this has no warm-up floor, so any non-zero rate is real starvation already surfacing as timeouts. **Must be zero before lowering any `DaemonMaxConnections` entry.** |
| `<daemon>_db_pool_empty_acquires_total` rate | warm-up only | none — diagnostic. Pools run `MinConns=0`, so the first acquisitions after boot necessarily count here. Read warm, as a rate, alongside the two signals above |
| `<daemon>_db_pool_acquire_wait_seconds_total` rate | ≈ 0 | none — diagnostic. Near-zero growth on a busy daemon is the positive evidence that a cap has headroom to give back |
| `<daemon>_db_notify_hub_conns` / `_subscribers` / `_channels` gauges | conns == 1, subscribers ≫ 1 | `<daemon>_db_notify_hub_conns == 0` while subscribers > 0 warn — `FAAS_DB_NOTIFY_HUB=0` is set and the daemon is back to one parked LISTEN connection per subscriber. Not an outage: the daemon's pool budget follows the switch (`db.DaemonMaxConnectionsNotifyHubDisabled`), so it is sized for that mode. It is a capacity signal — that daemon is consuming its pre-hub connection count against the fleet's `max_connections`, and the gap between `_conns` and `_subscribers` is the headroom the hub would return |
...
| **M5** | apid + Postgres + deploy pipeline with **prebuilt images only**; CLI (`gregale deploy --image`) | end-to-end: `gregale deploy` → parked → first request wakes; quotas enforced (plan matrix table-test) |
| **M6** | builderd + Railpack/Dockerfile in builder VMs | `gregale deploy` a bare Node and Python repo (no config) → live; OOM bomb build kills only its VM (tenant latency unaffected — measured); cache makes 2nd build ≥ 2× faster |
| **M7** | meterd + Polar (public-release billing provider, ADR-032 v2; Paddle/Stripe compatibility adapters): usage, quotas, overage, dunning; functions runtime (runner-node22, runner-node24, runner-python312, runner-python313, runner-go124, runner-go124-alpine); cron | invoice shadow equals hand-computed GB-h for a scripted 24 h scenario (< 0.1 % delta); function hello-world p95 wake < 1 s; Free-tier hard stop verified; **egress metering (ADR-046):** scripted guest egress produces a `net_tx_bytes` row whose sum equals the kernel `vethHost.rx_bytes` delta within 0 bytes, `GET /v1/usage` reflects `tx_bytes` and `net_tx_bytes`, and `billing.Provider.PushUsageRecord` is **not** called for the new columns |
...
| **M9** | Multi-box scale (ADR-025 axis 3 + ADR-066): cross-node live-instance migration, `OCIRegistryStorageBackend` end-to-end (ADR-054), per-node schedd + per-node placement (PR #509, Tier A4), Tier A6 migrating-instance watchdog (ADR-067). The native x86 target (`make native-m9-acceptance`) exercises the owner-scoped failure-safe heartbeat path and replaces the retired local target. ADR-066 flips `Proposed → Accepted`. ADR cross-refs: ADR-062 (per-node schedd, accepted 2026-08-16), ADR-063 (snapshot de-localization, accepted 2026-08-16), ADR-083 (active-passive standby, accepted 2026-08-16), ADR-110 (declarative split-box manifest, accepted 2026-08-16). | stop node B's vmmd on a native pair where each node runs schedd+vmmd: within `DefaultHeartbeatStaleness` (90s), the node reaches `lifecycle='unavailable'` and a `node.failed` event is present; the acceptance script runs `make leakcheck` before and after and restores vmmd. The live-migration and snapshot fan-out measurements remain follow-up M9 gates. |
...
| **M11** | apid handlers + DTOs + routes (12 endpoints, including explicit task retry) | 13 unit tests in `cmd/apid/handlers_jobs_test.go` PASS; byte-identical 404 contract for missing + cross-account |
...
### §14.A Workstream B — Multi-node failure-safe
...
| **WB-2** | Recovery arbiter (`pkg/sched/recovery_arbiter.go`) as single decision policy; live-migrator + dead-node-reconciler become tail-callers; `Engine.RecreateInstance` primitive with `recovery_recreate` state edge. Source-side migration leases persist in `migration_leases`; vmmd retains only the paused-VM handle cache. | 8-case table-driven test `pkg/sched/recovery_arbiter_test.go` passes; `pkg/sched/recreate_test.go` covers happy path + terminal-state skip + nil-engine tolerance. |
...
| **WB-5** | Operator runbook (`docs/runbooks/twonode-fault-injection.md`) + runbook-as-Go-test (`cmd/e2e/twonode_runbook_test.go`); two-node harness (`pkg/e2etest/twonode.go`) + fault injector (`pkg/e2etest/fault.go`). | The native heartbeat drill passes under `make native-m9-acceptance`; remaining workload and partition scenarios stay in the runbook follow-up. |

Source ADR: [ADR-137](adr/137-multi-node-failure-safe.md). Mega-PR: `failure-safe/mega2`. Operator drill: `docs/runbooks/twonode-fault-injection.md`. Gap row: `§17 G-Node-Recovery`.
...
faas/
  ...
  pkg/{api,state,fcvm,netns,oci,rootfs,meter,billing,wire}/
...
| G2 | **Customer secrets** — apps need env secrets; no encryption/injection/redaction design | `gregale secrets set KEY=…` → sealed with a host age key, stored encrypted in PG, injected into `/etc/faas/app.json` env at boot only, values redacted from build/app logs by pattern; never in snapshots of *other* deployments | **Closed by ADR-020 (PR #73 + M8 readiness PR for vmmd host-key lifecycle).** Sealed with X25519 host age key (filippo.io/age); injected into `/etc/faas/secrets.env` on every wake; values never cross the VM boundary in cleartext; vmmd generates/loads the host key at boot. |
...
| G5 | **CLI auth UX** — `gregale login` undefined | v1: browser-paste flow (dashboard shows key, CLI stores in OS keychain). OAuth device flow later | M5 — **Closed by PR #380 (issue #293):** keychain primary (macOS Keychain or Linux libsecret via D-Bus); plaintext-file fallback retained for headless hosts with a WARN recommending `gnome-keyring`; one-shot legacy-file migration on first successful keychain save. |
...
| G13 | **Stateless contract unenforced** — the platform is year-one stateless-only but a customer can `gregale deploy` a `postgres:16` base image, a Dockerfile with `VOLUME /data`, or a tarball with a top-level `data/` directory and only discover the data is gone after the first park. Today nothing in apid or imaged encodes "this platform is stateless" | **RESOLVED (Wave 0 PR-A, no ADR — the architecture is unchanged, we add a check):** new RFC 7807 code `CodeStatelessOnlyViolation` (422, mapped in `pkg/api/errors.go::StatusForCode`), apid tarball scan rejects `VOLUME`, `mkfs.ext4/xfs`, `mount -t ext4/xfs`, and top-level `data/`/`db/` at accept time (before any build slot is consumed), and imaged's `buildImageLayer` consults `pkg/imaged/base.go::StatefulDenyListMatch` against the resolved OCI ref (8 well-known stateful bases: postgres / redis / mysql / mariadb / mongo / cockroach / cassandra / clickhouse). Both paths feed `pkg/oci::SentinelToCode` → `deployments.error_code`. PR-B adds `gregale init --template={s3-uploader,slack-bot,rest-api-postgres,cron-worker}` to teach the pattern. **PR-C (ADR-047) under review (draft PR #421):** guest-init `fanotify` advisory on runtime writes to state-shaped paths (`/data`, `/db`, `/var/lib/postgresql`, etc.) shipped over AF_VSOCK DGRAM (`port=1025, msg_type=2`, distinct from `VsockResumePort=1024`) to the host, debounced 1s per `(path, mask-set)`. vmmd forwards each batch to apid over a new `/run/faas/apid.sock` gRPC seam (`api/proto/onebox/faas/apid/v1/advisory.proto`), which writes one `stateless.advisory` audit row via `pkg/audit.Auditor.Emit`. Surfaced at `GET /v1/audit-events?kind_prefix=stateless.advisory` (plus `?app_id=` and `?include_anonymous=` for the dashboard drill-down) and the new `db.NotifyStatelessAdvisory` SSE channel. Customer surface: `gregale audit-events [--kind-prefix=…] [--app-id=…] [--include-anonymous]`; `gregale tail --include-stateless`; dashboard `/dashboard/audit-events` (+ app_detail.html "Stateless advisories" link). Advisory-only — spec §17 G13 explicitly forbids EROFS for Wave 0. vmmd becomes a gRPC client for the first time; default-local stays DB-less (Manager.advisoryClient == nil is a no-op). | M8 |
...
| G-Node-Recovery | **Multi-node failure recovery is manual** — a node crash stranding live instances required operator intervention (manual `UPDATE compute_nodes SET active=false` + `UPDATE instances SET state='failed'` + redeploy). The dead-node reconciler marked stranded rows `failed` for billing correctness but didn't migrate them, and no cascade-drain existed for planned retirements | **RESOLVED (ADR-137, this PR):** migrations 00579 (`compute_nodes.lifecycle` ENUM with `active` as STORED GENERATED column), 00582 (recovery_audit columns), 00585 (deployments.snapshot_miss_count + backoff_until). `pkg/sched/recovery_arbiter.go` is the single decision policy (migrate vs recreate per-instance); `Engine.RecreateInstance` (Task #60) transitions stranded rows to `PARKED` via the new `recovery_recreate` state-machine edge. `pkg/sched/snapshot_backoff.go` stamps per-deployment backoff + returns `Retry-After` for the gateway. `cmd/gatewayd-public/compute_gateway_pool.go::WatchInvalidations` collapses the stale-route window from 7s to <500ms. `cmd/apid/handlers_compute_nodes_drain.go::postComputeNodeDrain` adds the `POST /v1/compute-nodes/{name}/drain?wait=1` operator surface. Operator drill in `docs/runbooks/twonode-fault-injection.md`; metal acceptance tests in `cmd/e2e/twonode_failure_safe_metal_test.go`. Closed gates: 5 PRs in this mega-PR sequence (M-recovery). | M-Recovery (Workstream B, §14.A) |
...
| V9 | Payment path & fees (provider fees) | Confirm Polar catalog and fee schedule for the incorporation country; retain provider-neutral fallback pricing in the sheet | Fee delta reflected in model before launch | pre-incorporation |
