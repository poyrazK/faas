...
rootfs switches from `golang:1.24-bookworm` (glibc) to
`golang:1.24-alpine` (musl). The Alpine variant remains smaller because it
uses musl, and both final images omit the Go compiler/toolchain.
...
The default `go124` (bookworm) is unchanged at the libc/runtime contract;
