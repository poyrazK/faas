...
- **Consequences:** The main workload always publishes its effective port. A sidecar publishes an endpoint when its legacy declaration or image metadata supplies a listener; init and worker sidecars may omit one. Explicit TCP port collisions are rejected because workloads share one namespace. Protocol-aware image listeners are defined by ADR-165; the endpoint registry projection is defined by ADR-167, while guest-side cross-VM transport, host ports, and public multi-port routing remain future work.
