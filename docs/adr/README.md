...
| 062 | Tier A per-node schedd + schedd-side async placement claim | accepted (revised 2026-08-16) | Phase 2 / Gate A |
| 063 | Tier A snapshot de-localization (residual local-cache semantics) | accepted (revised 2026-08-16) | Phase 2 / Gate A |
...
| 066 | Tier A5 cross-node live-instance migration (four-phase handoff: Park → mint lease → MigrateInstanceOwner → ack) | accepted (revised 2026-08-16; body says Accepted since 2026-08-07 — README row divergence fixed in Mega-PR-B Commit 6) | Tier A5 follow-up to ADR-062 deferred item 2 (live instances on the dying node) |
| 067 | Tier A6 migrating-instance watchdog (1 s ticker that self-heals stuck `state='migrating'` rows: re-invite active owner via gRPC, hard-delete dead owner) | accepted (revised 2026-08-16) | Tier A6 follow-up to ADR-066 §"Open follow-ups" item 1 |
