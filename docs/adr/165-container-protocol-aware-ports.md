...
- **Consequences:** TCP and UDP may use the same numeric port because they are separate socket namespaces; duplicate `(protocol, port)` tuples remain invalid within a task. The public readiness/HTTP port remains the existing `port` field. Host DNAT, public multi-port routing, and guest-side cross-VM transport remain separate follow-ups; the endpoint registry projection is defined in ADR-167.
