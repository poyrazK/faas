...
- **Decision:** Gregale defines one effective-state read model for a registered
  ...
  config-only diff remains backwards compatible.
- **Secret safety:** State and diff responses never contain secret plaintext,
  ...
  reported as `unknown`, never equal. A future environment clone may copy a
  sealed customer envelope only inside the control plane; it must recreate a
  managed binding and may not copy provider-issued credential bytes.
...
- **Consequences:** The project diff CLI now uses the unified endpoint. Runtime
  ...
  existing app env-diff contract. Secret versions for customer-managed secrets
  remain a follow-up because the current schema stores fingerprints and update
  timestamps but no monotonic version. This ADR adds no database migration.
