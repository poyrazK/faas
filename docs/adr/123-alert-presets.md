...
`alert_presets` is a fixed 8-row table seeded by migration `00353`. Rows
...
`00352_alert_presets.sql` enforces the closed-set CHECK constraints
...
3. `migrations/00354_alert_rules_extend_metrics_chk.sql` (DROP + ADD
...
- **`cert_expiry_seconds`** — PromQL path; the gauge
  ...
  rule and lives in `migrations/00356_meterd_…`).
...
- ADR-123 follows the cors_presets precedent exactly (slot 00352, hand-
...
Top-down:
- `migrations/00352_alert_presets.sql` + `00353_alert_presets_seed.sql`
