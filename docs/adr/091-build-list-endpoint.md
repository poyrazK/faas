...
8. **Schema change: `builds(deployment_id, started_at desc nulls last)`.**
   ...
   migration is `migrations/00197_builds_deployment_started_idx.sql`.
...
| Migration | `migrations/00197_builds_deployment_started_idx.sql` | NEW. Index. |
| Migration test | `migrations/00197_builds_deployment_started_idx_test.go` | NEW. Apply-walk pins it. |
...
> 2. `00168_reserve_slot.sql` (another sibling fence) + 169, 170, 171,
...
- `migrations (contiguity + apply)` — green. The new
  `00197_builds_deployment_started_idx.sql` runs cleanly forward
